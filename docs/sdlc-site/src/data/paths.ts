...
const frontSteps: FlowStep[] = [
  ...
  gateStep("architecture", "architecture.md", "count: 3 approvers = base 1 + contract risk 2 — capped at active people − 1 · enforced (base only when not counted)", "#ca6f1e"),
  {
    id: "ui",
...
const buildSteps: FlowStep[] = [
  ...
  },
  {
    ...
    description:
      "Wire an advisory AI first-pass (CodeRabbit, never the authority), record the human engineer review (the same count: 1 approver, who should not be the author, with high risk, a contract surface or a high directory on the risk map raising the advisory count), and on merge record the ship in build-log.json.",
    actor: "engineer",
    status: "merged",
