...
export const CLI_COMMANDS: CliCommand[] = [
  ...
  { constant: 'DEFER', value: 'yad defer', target: 'setup', category: 'front', description: 'Set an optional step aside to do later: yad defer <epic> <step> --reason "<why, and who is waiting>". Same steps as yad skip, same refusals. The step and its review gate are marked deferred with the reason, who and when recorded; the chain walks past them while their review stays visibly owed. A skipped step cannot be deferred, nor a deferred one skipped.', visibleTo: ALL },
  { constant: 'UNDEFER', value: 'yad undefer', target: 'setup', category: 'front', description: 'Put a deferred step back in the chain: yad undefer <epic> <step>. Removes the deferral and its record. Refused once the step that follows it is finished or work past it has started; picking a step up behind finished work is not supported yet.', visibleTo: ALL },
