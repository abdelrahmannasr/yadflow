...
export const CLI_COMMANDS: CliCommand[] = [
  ...
  { constant: 'EPIC_NEW', value: 'yad epic new', target: 'setup', category: 'front', description: 'Start an epic — the engine writes its lifecycle. Seeds epics/EP-<slug>/.sdlc/state.json with the step chain of a lifecycle profile (--profile classic|analysis-first, default classic), plus empty approvals/comments ledgers and reviews/. --type feature|chore; a change/defect/hotfix threads off an existing epic, so yad-change seeds those, and the discovery front-zero stays yad-discovery\'s. Writes no epic.md, no branch and no commit, and refuses an epic that already has a state.json.', visibleTo: ALL },
