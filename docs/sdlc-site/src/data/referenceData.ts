...
export const DECISION_TREE: DecisionBranch[] = [
  ...
  },
  {
    ...
    result: 'full count = base 1 + risk step, capped at active people − 1',
    detail: 'The risk step is +2 for a contract tag and +1 for auth or payments — the highest tag, never the sum. It counts people, not roles. The gate caps the count at the active people less one, never below 1, and enforces the capped number: a contract gate asks 3, and with 2 active people it asks 1 (one seat is left for the author). The number of active people is counted live and printed next to the ask: committed or approved, from the approval and ship records plus git authorship in the product and every connected repo, over a window that scales with how fast the team merges (the last 20 merged PRs, bounded 30-180 days, and the wide end when there are fewer than 20). Nothing is stored. A source that cannot be read makes it "not counted" rather than a number, because a small count lowers what a gate asks for. Then no cap applies: the gate holds only on the base and prints the rest as a shortfall (for example "1 short"). Product CI is that case on a Product with connected repos. Without the cap a two-person team could never pass its architecture gate.',
    visibleTo: ALL,
  },
  {
