...
export const CLI_COMMANDS: CliCommand[] = [
  ...
  { constant: 'HISTORY', value: 'yad history list|show|search', target: 'build', category: 'build', description: 'What the Product has done (E20), read live from each work item\'s own files on any branch, writing nothing. list: every work item, open and finished, newest first, with --type/--theme/--thread/--open/--done. show <id>: every step with its state and closing record, and the approvals recorded for each review step. search <text>: ids, titles, themes, types, repos, and who closed or merged a step, its PR and its commit. --json on all three.', visibleTo: ALL },
