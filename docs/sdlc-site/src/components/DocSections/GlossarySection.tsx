...
const TERM_GROUPS = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Approval count — how many distinct people a step asks to approve: base 1 + risk step, capped at the active people less one (never below 1). Only the base is enforced until E73 — a later yadflow change that adds yad gate lower --reason, a recorded way out of a gate a team cannot meet; the gate prints the rest as a shortfall against the capped count.',
      'Risk step — what a step\'s risk tags add to the count: contract +2, auth or payments +1, the highest tag and never the sum. Advisory until E73, which enforces it capped by the active people together with a way out (yad gate lower --reason).',
      'Active people — how many people committed or approved lately, counted live from the approval and ship records plus git authorship, never from a stored list. The window scales with how fast the team merges. Printed once per epic on `yad gate status` and `yad gate sync`, and beside the ask in a generated review PR. It caps each gate\'s count at the active people less one — one seat is left for the author — and the cap is reported, not enforced, until E73. An unreadable source reads as "NOT COUNTED" on `yad gate status` and `yad gate sync` ("not counted" in the review PR), never as a small number, and then no cap is shown.',
