...
> (`sdlc-run`), the trust log, per-story build-state, and earned `checks` (Step B). Phase 4b adds the
...
`sdlc-run` already **walks every back step generically** (`spec → tasks → implement → checks`) reading
...
|------|--------|----------------------|-----------------------|------------|-------------|
| `spec` | `specs/<story>/` (spec/plan/tasks) | human approves the spec untouched | human edits the generated spec before approving | human rejects / re-runs the ceremony | the spec review (reuse `sdlc-review-gate` on the spec artifact) |
| `tasks` | `tasks.md` (atomic task list) | task list implemented as generated | a task's `Files:`/scope edited before implementing | task list discarded / regenerated | the moment `sdlc-implement` first consumes a task (the list "survived contact") |
| `implement` | task diff on a branch | merged as authored | engineer edited the diff before merge | scope overrun / contract touch / checks FAIL | the engineer review at `sdlc-ship` (already records this in 4a) |
...
- `tasks` is derived **mechanically from an already-approved plan/spec** (the heavy `sdlc-spec`
...
- **What to build:** the tasks-survived-implementation trust hook (above); accumulate `tasks` runs;
  once the slice clears the threshold, `set-dial step: tasks to: machine_advance` lets `sdlc-run`
...
- **Halts unchanged:** if the spec is not approved, or the generated task list is empty/ambiguous,
  `sdlc-run` halts and pulls in a human regardless of dial.

### Step D — automate the `implement → check` handoff
- After `sdlc-implement` produces a committed task branch, **auto-run the check gates** instead of
  waiting for a human to trigger `sdlc-checks`. In 4a's loop this is exactly `implement`'s dial being
...
- **Scope guard stays hard (the central protection):** if the diff grows beyond the task's declared
  files, or touches the contract surface, `sdlc-implement` halts and `sdlc-run` blocks — regardless of
...
3. **No engine change** — `sdlc-run`'s loop, `set-dial`, the threshold predicate, the kill switch, and
...
- `spec` and `tasks` runs are recorded in `trust-log.json` with a defined, human-anchored verdict;
  `sdlc-status` surfaces them alongside `checks`/`implement`.
...
- No `tasks`/`spec` evidence was fabricated to unlock automation; Step C is enabled only on genuine
  ...
  `human_approve` with the reason shown in `sdlc-status`).
