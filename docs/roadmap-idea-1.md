...
| E6 | Skill binding leaves the code, allows multiple skills per step. **The catalogue keeps its `skill` column as the shipped default**; `.sdlc/skills.json` overrides it per step, written by `yad skill bind` / read by `yad next`, `yad epic new` and the Build lanes. Several skills are a chain (decision 7) with a cost warning. No file shape moved — the binding file is new, not a changed one. Left to E51: a per-profile layer between the project's file and the catalogue, once E50 can detect installed skills | M | E4 |
