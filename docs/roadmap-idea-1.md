...
worked in those directories lately. Tier 1's cap is still E72.
...
| E72 | Base + risk step, capped by capacity. Show and record every cap. **Left by E66:** the Build count is printed by `checks/` scripts that run from the PR's own checkout, so a PR can edit the script that counts it — this row must answer that before the count holds a merge | S | E66 + E71 |
