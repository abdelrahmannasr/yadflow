...
(E71) and the cap is printed and recorded; only the base holds a gate. E73 warns when a gate may not pass; E108 will enforce the
capped count beside `yad gate lower --reason`, once the count is accurate. The Build count stays reported — branch protection holds a Build merge.
...
| E68 | Suggest reviewers from history; read CODEOWNERS as a hint only. **E67 already answers the narrow half** — who committed in the `high` directories a change touches, in 30 days, as an ask the engineer review reports. This row is the general one: any touched directory, a suggestion rather than an ask, and CODEOWNERS read as a hint. Reuse `recentAuthors` / the `WHO` lines rather than a second history reader | S | E62 + E67 |
