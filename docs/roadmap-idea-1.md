...
`payments/**` above is an example of such a directory, not a tag.
...
| E67 | Escalate by proven history — a live git query at gate time, nothing stored. **Since E65 the question is per directory:** an approval from someone who committed in the last 30 days to the `high` directories the change touches, not to a named topic | S | E65 |
