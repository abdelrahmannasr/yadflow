...
(E71) and the cap is printed and recorded; only the base holds a gate until E73 enforces the capped count
beside `yad gate lower --reason`. The Build count stays reported — branch protection holds a Build merge.
...
| E73 | Unmeetable-gate detection and `yad gate lower --reason`. **Also turns on the capped count E72 only reports** — E72 decision 3. Its cases are E72's review findings (a)–(h): the name/login double count, the GitLab bot, connected-repo committers, the wide window, a count that grows before the merge, a verdict found only after the merge, a machine-dependent verdict on a local ledger, and `yad mode solo` as the only way out today. **Open for E73:** once the capped count is enforced, what does an UNKNOWN count mean — the base alone (today's rule) or the full count? E72 decision 1 only covers the reported cap. Tests named `E73 FLIPS THIS` pin "a known count is reported, not enforced" and are the ones E73 turns around | S | E72 |
