...
(E71) and the capped number holds a Shape gate; when the people cannot be counted, no cap applies and only
the base holds. The Build count stays reported — branch protection holds a Build merge.
...
| E72 | Base + risk step, capped by capacity. Show and record every cap. **Left by E66:** the Build count is printed by `checks/` scripts that run from the PR's own checkout, so a PR can edit the script that counts it — this row must answer that before the count holds a merge. **Four decisions were the user's (2026-09-21).** (1) **An unknown count applies NO cap, and only the base holds.** `gateCapFor(rule, active)` returns `null` for anything that is not a whole count, and the predicate then does exactly what it did before this row: base 1 enforced, the risk step reported as `short`. The two other answers each broke a rule already closed: enforcing the uncapped count is E7's two-person deadlock, and capping on an unknown invents a number, which E71 forbids. **The cost, stated:** Product CI checks out the hub only, so `connectedRepos` reads every connected repo as "not on disk" and `active` is `null` there — and CI is the only writer of `state.json` on a verified Product. So on a verified Product with connected repos, **CI enforces the base alone** and E72 adds no teeth there until CI can count people. The brief expected CI to be the STRICTER side; with this answer it is the more lenient one. Every surface says it: `activeSum` now ends `— no cap applies, so only the base holds each gate`. (2) **"Record" is an optional `capped: { needed, to, active }` key on the step's closing record** (E18), beside `waived`, written by `gate sync` only when the cap LOWERED the count. No shape change — the E10 `waived` precedent: an older release ignores a key it does not know. `gate status` prints it back (`count capped from 3 to 1 (2 active people)`). A cap that lowered nothing, or an unknown count, records nothing. (3) **The capped count is ENFORCED, and E73 lands next.** Rule 7 is not met by this row alone: E71 over-counts on purpose (one person with two commit addresses is two people), so a real team of two read as three asks a contract gate for two approvals, and only one person is not the author. The predicate runs after the review PR merges, so that gate is a merged PR stuck `in_review` with no way to add approvals. The way out is E73's `yad gate lower --reason`. This is acceptable ONLY because nothing between E72 and E73 is released (E62's precedent) — do not move `next` between them. A test pins the jam so E73 has a case to turn green. (4) **The Build count stays REPORTED, and that is the answer to E66's note.** A Build merge is held by the platform's branch protection (Part 9), never by this count, so the CI half running from the PR's own checkout stays harmless: nothing blocks on it. `yad open-pr` SHOWS the capped count (`capped to 1 for 2 active people — base enforced, risk step advisory`); `risk-map-check.sh` and `risk-route.sh` cannot cap — a code repo's CI has no Product — and now say so instead of "until the capacity cap". If the Build count ever holds a merge, E66's note comes back first. **Settled from the code, not asked.** The arithmetic: `to = max(base, min(needed, max(1, active − 1)))`. The base is 1 and the floor is 1, so the cap can only ever trim the RISK STEP — E62's one approval still holds on every team gate. **The `− 1` is one seat, not a check:** the author wrote the review branch in the Product, whose history is in the count, so they are always among `active`; the engine never learns who the author is, and whether the author's own approval counts is the platform's rule (E62 decision h). **Solo mode** computes and prints the cap and enforces nothing (`short` stays 0), as E7 and E71 print in solo. `short` now measures against the number the gate ASKS for — the capped count when `active` is known, the full count when it is not — so under a cap `short` is exactly what `missing` holds. **Surfaces:** `gate sync` and `gate status` (`— capped to 1: 2 active people, less one seat for the author — enforced`, `— enforced in full`, or unchanged `— base enforced, risk step advisory` on an unknown); the review-PR body's `Approvals needed` line (the capped number, dated, and naming what happens where the people cannot be counted — the gate counts again when it decides); the `--json` bundle (`step.cap`, an object); `hub-route.sh` (it cannot count, so it says what the gate does). `gateStatus` takes an optional precomputed `headCount`, as `gateSync` already did, so a test can show the cap on a fixture with no git history. **Golden:** `gates` gains `cap: null` on all nine; pass/fail, `missing` and `short` are unchanged, because the frozen snapshot passes no count. **Costs, stated:** `risk-map-check.sh`, `risk-route.sh` and `hub-route.sh` change wired bytes, so every wired repo reads `outdated` until `yad update`. **Not here:** `yad gate lower --reason` and unmeetable-gate detection (E73); counting people in Product CI | S | E66 + E71 |
