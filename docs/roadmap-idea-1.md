...
| E7 | Gate rules per step — **people-free by design**: base + risk step only, never a name or a role. The capacity cap arrives in E72. **Shipped as `gateRuleFor(step)`**: `needed = base + risk step`, read from the risk tags the EPIC records (rule 3 — the file wins, the same discipline as `optionalStepsFor`), counted in distinct PEOPLE. Two inferences this row left open, now closed in code: `auth`/`payments` ARE the high tier (+1) and `contract` keeps its own larger step (+2), and several tags take the **maximum** step and never the sum — a gate is one decision about the riskiest thing it touches, and summing would ask a three-tag step for five approvals. **`base` is 1, not today's effective 2.** E7 lands WITHOUT the cap (E72) and without `yad gate lower` (E73), so a base of 2 would make a single-repo contract gate need four approvers on a team that has three available, with no recorded way out — which rule 7 forbids. **The roster-era rule is NOT replaced, it is ANDed.** The old rule reads roles (1 owner, 1 reviewer, a domain-owner per touched domain); the new one reads a number. Requiring BOTH is what keeps E7 from loosening anything: dropping the per-repo domain-owner requirement here would have weakened the stories gate from 3-4 people to 1 in the window before the proven-history tier (E67) exists. The role rule dies with the roster (E62/E64), and whoever deletes it then decides where `base` settles, with the cap in hand. **What stays name-based on purpose:** `isEscalated`'s `stories-review` clause (a step name inside a rule) — it triggers the role rule only, and E62 retires both together. **Where the count binds that the role rule cannot see:** one person holding two roles is two roles and one approver. That is the case the new tests pin; every other fixture in the suite has two distinct approvers, so without them the count would never be the reason anything failed (the invisible-rule trap). **Rule 6 surfaces:** `gate sync`, `gate status`, `gate --json` and the generated review-PR body all print the arithmetic (`2 of 3 approver(s) — base 1 + contract risk 2`), never a bare number. **Golden:** pass/fail is identical on the frozen v3 project (its contract gate holds four approvers against a rule needing three); the snapshot's `gates` section gains `base`/`riskStep`/`needed`/`risk`/`have`, and `have: null` on an inherited or skipped step records that nothing was counted, which is not the same fact as zero approvals | M | E4 |
