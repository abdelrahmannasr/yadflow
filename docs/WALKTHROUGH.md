...
that arithmetic. **Only the base holds the gate** until E73; the rest is reported as a shortfall. The
...
A team gate that passed while the cap lowered its ask records `capped: { needed, to, active }` on its
closing record.
