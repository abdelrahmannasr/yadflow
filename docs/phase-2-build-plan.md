...
Builds directly on Phase 0 (research) and Phase 1 (the `yad` module: author-epic, review-gate, status, dials, gate rule, worked demo). Same rules apply: talk to tools through their interfaces, all state in files, front states never auto-advance, smallest-useful-first.
...
> **Later addition:** an **optional** `analysis → analysis-review` discovery step (`yad-analysis`) may now precede the epic. When run, it assigns the `EP-<slug>` ID and seeds the chain; when skipped, the epic step shapes the idea inline exactly as below. See `skills/yad-analysis/SKILL.md`.

---

## What to build, in order

### 1. `yad-architecture` — front state 3
...
### 2. Reuse `yad-review-gate` for the architecture review
...
- **One reusable gate.** All three new reviews (architecture, UI, stories) use the Phase 1 `yad-review-gate`. If any of them needs gate code copied or forked, stop — fix the gate to take options (reviewer routing, escalation trigger) instead. Proving the single gate covers all four reviews (epic + 3 new) is a primary success criterion.
...
- Three new skills authored in `skills/sdlc/` in the verified v6.8.0 format: `yad-architecture`, `yad-ui`, `yad-stories`.
