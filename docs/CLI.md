...
`yad mode team`. Team mode reads no count here, so it costs nothing; in solo mode `yad next` and
`yad doctor` now read the git history of every connected repo once per run.
...
| Someone approved a review in the window, and more than one person is counted | On GitHub an author cannot approve their own work, so an approval shows a second person. On a local ledger they can, which is why one person counted is not enough | ``! solo mode is on, but someone approved a review in the last 90 days, so more than one person may work on this Product. If so, `yad mode team` makes each review gate ask for approvals, which solo mode waives`` |

When the count is unknown (a connected repo is not on disk, for example), nothing is suggested, and the
line says so: `the people could not be counted (<why>), so no switch to team mode is suggested`.
`yad gate status` does not repeat it, because its `active people: NOT COUNTED` line already says it.
`yad mode --json` carries the same answer as `suggest` (`{ known, line }`, or `null` in team mode).

**Known limits.** The line says "may", because each of these reads as more than one person when it is
not, or no longer is: one person with two platform accounts; one person whose git name has two spellings
(`ada` and `Ada Lovelace`); a robot that commits under a plain name with no `[bot]` at the end (for example `semantic-release-bot`, or `github-actions` set by hand in a workflow), which counts as a name; someone who committed to a connected repo but cannot approve Product reviews;
someone who left but is still inside the counting window (up to 180 days on a young Product); and, on a
local ledger, one person's own approval recorded under a different name from their commits.
