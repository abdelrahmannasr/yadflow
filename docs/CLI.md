...
| A person is shown as their git **name**, plus `(@login)` only when their commit address is a platform `noreply` one. A git name that is itself an e-mail address is shown as the login, or as `a name that is an e-mail address`. | yadflow prints no e-mail addresses. A work address names no login. |
