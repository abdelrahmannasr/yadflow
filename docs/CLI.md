...
| `yad docs build` / `deploy` / `sync --refresh` | a failed npm install or build exited 0 and answered `ok: true`, `{ action, built }` | exit 1 and a refusal naming the site; `sites` says what happened to each site (a change of the plain exit code too) |
