...
(`yad-pr-template` installs it). `high` risk (or a touched contract / auth / payments surface) raises
the approval count the same way `yad-review-gate` does — only one approval holds the merge for now, and
