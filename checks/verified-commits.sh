...
# appear in the .sdlc/verified-authors allowlist (generated by `yad check --fix` from the Product
# roster's `email` fields + hub.json `verified_authors`). Unsigned commits and commits from unknown
# authors do not reach merge — on the Product and on every connected repo alike.
#
...
if [ -f "$ALLOWLIST" ]; then
  ...
  echo "WARN [verified-commits]: ${ALLOWLIST} not found — author allowlist NOT enforced. Add 'email' to the Product roster (or hub.json 'verified_authors'), then run \`yad check --fix\`."
fi
