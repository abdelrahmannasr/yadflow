...
3. **Active gate** — for the `currentStep` (if it is a `review+approve` step), compute and show:
   ...
   - **A gate that may not pass (E73):** under a team gate that has not passed, `yad gate status` can
     print a warning line. `! may not be met: …` means the one required approval may have nobody but the
     author to give it. `! if the risk step were enforced: …` is a what-if: the extra approvals that risk
     tags add could never all be given by the people counted, if they were enforced. Relay it as stated,
     as a warning and never as "blocked": it holds nothing, and the gate can still pass today.
