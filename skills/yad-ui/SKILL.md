...
**File-only mode — write it.** In `state.json`: set `ui-design.status: "done"`, set
`ui-design-review.status: "in_review"`, and set `currentStep: "ui-design-review"`. Write
`state.json`. Do **not** touch `approvals.json`. `yad gate open` makes the same edit, so this is a
no-op once the gate has run.
