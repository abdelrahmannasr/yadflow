...
```json
{
  ...
  "gate_sync_version": "3.15.3",                              // OPTIONAL exact pin for the wired gate-sync job; must be an exact 3.x.y (anything else is skipped). Omitted => the .sdlc/cli-version.json stamp if that is a valid 3.x, else floating 3
