...
> the move cannot be made by a human commit, so the gate bot makes it: `yad gate ci` moves the folder on
> its next run on the default branch, once the checks committed in the repo know `foundation/`. A product has ONE product level: never seed a Foundation beside an `EP-discovery`.
