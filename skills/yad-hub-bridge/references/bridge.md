...
(`risk_tags: ["contract"]` → 3 approvers, capped at the active people less one; only the base 1 when
the people cannot be counted) is unchanged.
...
- **Merge → advance.** When the count is met (the count capped at the active people less one; only the base, 1 distinct approver, when the people cannot be counted; solo mode waives it), every thread is resolved, **and the review
