...
- **Changing the contract** (exceptional) — altering the agreed shape itself (new field crossing repos,
  ...
  raise the full approver count, capped at the active people less one, E72; only the base holds when the people cannot be counted), and only then implement — recording `Contract-Change: yes` so the §C
