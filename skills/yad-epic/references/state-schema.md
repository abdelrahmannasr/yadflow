...
| `status` | the same **step states** as the Shape chain | Lifecycle. `yad-run` advances `done` steps and stops the lane on a halt. This file is **not** migrated to shape 7 — the skill writes it, not the engine, so a rewrite would be undone by the next write. `todo` and the old `blocked` both read as "not started" here, exactly as they do in `state.json`. |
