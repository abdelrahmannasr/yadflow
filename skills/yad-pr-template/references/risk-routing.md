...
high step for a change that touches a `high` directory, reading the map from the base branch. A `guessed`
level counts there as a `confirmed` one does: a level only raises the count, never lowers it.
