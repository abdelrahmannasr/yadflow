...
test('templateBody: hub-tooling emits the code-task shape (fixes #80) and fills the risk', () => {
  ...
  // it must NOT carry the artifact-review markers the Product's own template has
...
// `--profile hub` is passed by a workflow committed in the USER's repository, and that workflow and
// the script it calls are refreshed by different commands with no ordering between them. `product` is
// the new spelling; both must pass, or whichever half of the upgrade lands first breaks their checks
// until someone re-wires by hand.
const runProfileGate = (script, args) => {
  try { execFileSync('bash', [path.join(ROOT, script), ...args], { stdio: 'pipe' }); return { code: 0, out: '' }; }
...
for (const profile of ['hub', 'product']) {
  test(`pr-template: --profile ${profile} is accepted and behaves identically`, () => {
    ...
    fs.writeFileSync(bodyFile, [
      ...
      '## Impact & Risk (front-half)', '', 'Risk tags: contract', '',
      '## Checklist', '', '- [x] reviewed', '',
    ].join('\n'));
    const r = runProfileGate('skills/yad-pr-template/templates/checks/pr-template.sh',
      ['--profile', profile, '--head', 'review/EP-demo/architecture', bodyFile]);
    assert.equal(r.code, 0, `--profile ${profile} was rejected:\n${r.out}`);
    fs.rmSync(T, { recursive: true, force: true });
  });
  ...
  test(`pr-title: --profile ${profile} accepts a review title`, () => {
    const r = runProfileGate('skills/yad-pr-template/templates/checks/pr-title.sh',
      ['--profile', profile, '--head', 'review/EP-demo/architecture', 'review: architecture (EP-demo)']);
    assert.equal(r.code, 0, `--profile ${profile} rejected a valid review title:\n${r.out}`);
  });

  test(`pr-title: --profile ${profile} still REFUSES a non-review title on a review branch`, () => {
    const r = runProfileGate('skills/yad-pr-template/templates/checks/pr-title.sh',
      ['--profile', profile, '--head', 'review/EP-demo/architecture', 'chore: sneak past the review gate']);
    assert.equal(r.code, 1, `--profile ${profile} let a code title through the review gate:\n${r.out}`);
  });
}
...
test('runOpenPr never lets the HUB default_branch become a code repo\'s base (#168)', async () => {
  ...
  try {
    ...
    assert.notEqual(seen.base, 'trunk', 'the Product trunk never leaks into a connected repo\'s PR');
  } finally {
    process.exitCode = prev;
    fs.rmSync(T, { recursive: true, force: true });
    fs.rmSync(bare, { recursive: true, force: true });
...
// ---------------------------------------------------------------------------------------------
// The count is REPORTED, not enforced: the roster-era role rule (pinned by the tests above) is still
// the only thing that decides `passed`, because the roadmap's tier-1 rule includes a capacity cap that
// does not exist until E72, and enforcing the uncapped half would make a two-person team's contract
// gate unpassable. So these tests pin what the count SAYS — `gateRule`, `have`, `short` — and pin that
// it changes no verdict. The case they are built around is the one the role rule cannot see: one person
// holding two roles is two roles and one approver.
...
test('gateRuleFor: the tier NAME comes from the winning tag, not from the number', () => {
  // If `auth` ever moves to +2, an auth-only step must not start printing "contract risk 2".
  assert.equal(gateRuleFor({ risk_tags: ['auth'] }).risk, 'high');
  assert.equal(gateRuleFor({ risk_tags: ['auth', 'contract'] }).risk, 'contract');
  assert.equal(gateRuleFor({ risk_tags: ['contract', 'auth'] }).risk, 'contract', 'tag order does not decide the tier');
});

test('gateRuleFor: a hand-edited risk_tags of the wrong TYPE is read as no tags, not a crash', () => {
  // `risk_tags: "contract"` (a string where the shape says array) is a plausible hand edit, and the gate
  // that was about to report on the step must still report.
  assert.deepEqual(gateRuleFor({ risk_tags: 'contract' }), { base: 1, riskStep: 0, needed: 1, risk: 'normal' });
  assert.deepEqual(gateRuleFor({ risk_tags: { contract: true } }), { base: 1, riskStep: 0, needed: 1, risk: 'normal' });
});
...
test('gatePredicate: a hand-added risk tag raises what the count asks of an ordinary step', () => {
  // The tags are read from the EPIC's recorded step, not from the catalogue — a team that marks
  // `epic-review` as touching auth has raised what the count asks for, and the engine honours the file.
  ...
  const tagged = gatePredicate({ step, approvals: onePerson, currentHash: 'sha256:H1', merged: true, threadsResolved: true });
  assert.equal(tagged.gateRule.needed, 2);
  assert.equal(tagged.gateRule.risk, 'high');
  assert.equal(tagged.have, 1);
  assert.equal(tagged.short, 1);
  // The same approvals on the same step WITHOUT the tag are not short: the tag is what moved the number.
...
test('gatePredicate: solo reports no shortfall, and still says what team mode would ask', () => {
  ...
  assert.equal(p.short, 0, 'a waived gate is not "short" of anything');
});
...
test('gatePredicate: a waived path reports `have: null`, which is not the same as zero approvals', () => {
  ...
  assert.equal(i.gateRule.needed, 3);
  ...
  assert.equal(k.gateRule.needed, 1);
});
...
// Containment is to the WORKSPACE (the Product root's parent), not the Product root: the standard layout puts
// code repos BESIDE the Product (project/{product,backend}), so `../backend` must register (issue #129).
...
test('registerRepo accepts a sibling repo inside the workspace, and stores the path as typed', () => {
  ...
  const T = path.join(WS, 'product'); // the Product root
  fs.mkdirSync(T);
...
test('registerRepo rejects paths that resolve outside the workspace', () => {
  ...
  // the workspace DIRECTORY itself contains the Product — registering it would aim repomix and the CI
  // writes at the whole tree. Strict descendants only (plus the Product root itself, for a monorepo).
  git(WS, 'init', '-q');
...
test('stepStatus: every state in the model, and the three legacy encodings it translates', () => {
  // Each of the eight rows reads back as itself when the file simply says so.
  for (const row of STEP_STATES) {
    const step = { id: 'x', status: row.id, ...(row.record ? { record: REC } : {}) };
    assert.equal(stepStatus(step), row.id, `${row.id} reads back as itself`);
  }

  // The three legacy encodings. Each is a FLAG BESIDE `status: 'done'` — which is how both writers
  // have always written them — or the old spelling of "not started".
  assert.equal(stepStatus({ status: 'done', inherited: true }), 'satisfied');
  assert.equal(stepStatus({ status: 'done', skipped: true }), 'skipped');
  assert.equal(stepStatus({ status: 'blocked' }), 'todo', 'a `blocked` with no record is the pre-shape-7 spelling of `todo`');

  // THE RECORD IS THE DISAMBIGUATOR. One word, two meanings, told apart by whether anything is
  // recorded on it — and by nothing else, so no reader needs a schema version in hand.
  assert.equal(stepStatus({ status: 'blocked', record: REC }), 'blocked');
  assert.equal(stepStatus({ status: 'blocked', record: {} }), 'todo', 'an empty object is not a record');
  assert.equal(stepStatus({ status: 'blocked', record: { reason: '   ' } }), 'todo', 'nor is a blank reason');

  // A FLAG WITHOUT THE `done` BESIDE IT IS A HAND EDIT, and must not finish a step. This is the one
  // a first cut of the model got wrong: read the flag alone and `skipped: true`, typed into a file by
  // somebody with no route permission to skip anything, would unblock every step behind it.
  assert.equal(stepStatus({ status: 'blocked', skipped: true }), 'todo');
  assert.equal(stepStatus({ status: 'in_review', inherited: true }), 'in_review');

  // Unknown reads as null — the file wins, and doctor reports it.
  assert.equal(stepStatus({ status: 'quantum' }), null);
  assert.equal(stepStatus({}), null);
  assert.equal(stepStatus(null), null);
});

test('isPassed / isAuthored: the two questions the old `status === \'done\'` was answering at once', () => {
  const expect = {
    todo: [false, false],
  ...
  // The table drives the test, so a row added to STEP_STATES with no answer here fails rather than
  // passing unexamined.
  assert.deepEqual(STEP_STATES.map((r) => r.id).sort(), Object.keys(expect).sort(),
    'every state in the model has a pinned answer here');
  for (const row of STEP_STATES) {
    const step = { id: 'x', status: row.id, ...(row.record ? { record: REC } : {}) };
    assert.deepEqual([isPassed(step), isAuthored(step)], expect[row.id], `${row.id}`);
    assert.deepEqual([row.passed, row.authored], expect[row.id], `${row.id} — and the table agrees`);
  }
  // Fail closed on a word this release cannot name: an unknown state never lets a chain past it.
  assert.deepEqual([isPassed({ status: 'quantum' }), isAuthored({ status: 'quantum' })], [false, false]);
  // Only `done` is authored, so only `done` has an artifact to audit or a stranded author step to repair.
  assert.deepEqual(STEP_STATES.filter((r) => r.authored).map((r) => r.id), ['done']);
});

test('claimsSkipped / claimsInherited: what the FILE says, which is not what the chain concludes', () => {
  // The forged flag from the test above: not skipped as far as the chain is concerned, and still a
  // claim — which is what `gatePredicate`'s route guard and `yad doctor` both need to see to refuse it.
  const forged = { id: 'stories-review', status: 'blocked', skipped: true };
  assert.equal(stepStatus(forged), 'todo');
  assert.equal(isPassed(forged), false);
  assert.equal(claimsSkipped(forged), true);
  // Both spellings, shape 7's and the legacy one.
  assert.equal(claimsSkipped({ status: 'skipped' }), true);
  assert.equal(claimsSkipped({ status: 'done', skipped: true }), true);
  assert.equal(claimsSkipped({ status: 'done' }), false);
  assert.equal(claimsInherited({ status: 'satisfied' }), true);
  assert.equal(claimsInherited({ status: 'done', inherited: true }), true);
  assert.equal(claimsInherited({ status: 'done' }), false);
});

test('isStepRecord: a record has to say why, and only `reason` is required', () => {
  assert.equal(isStepRecord({ reason: 'no UI on this epic' }), true);
  assert.equal(isStepRecord({ reason: 'x', by: null, date: null }), true, 'attribution is best-effort');
  assert.equal(isStepRecord({ by: '@al', date: '2026-08-01' }), false, 'who and when without why is not a record');
  assert.equal(isStepRecord({ reason: '' }), false);
  assert.equal(isStepRecord(null), false);
  assert.deepEqual(RECORDED_STEP_STATES, ['skipped', 'deferred', 'satisfied', 'blocked']);
  assert.equal(stepStateDef('deferred').passed, true, 'a deferred step does not hold the chain — that is what deferring is');
  assert.equal(stepStateDef('nope'), null);
});

test('the chain readers walk past every passed state and stop on a blocked one', () => {
  const chain = (...steps) => ({ epicId: 'EP-x', currentStep: steps.find((s) => !s.status.match(/done|skipped|satisfied|deferred/))?.id || 'ready-for-build', steps });
  const S = (id, status, extra = {}) => ({ id, type: id.endsWith('-review') ? 'review+approve' : 'author', artifact: `${id}.md`, status, ...extra });

  // Every finish-without-authoring state lets the chain through, so `stories` is ready.
  const mixed = chain(
    S('epic', 'done'),
    S('epic-review', 'satisfied', { record: { reason: 'reviewed in the parent', by: null, date: null, link: 'EP-parent' } }),
    S('architecture', 'skipped', { record: { reason: 'no new surface', by: '@al', date: '2026-08-01' } }),
    S('architecture-review', 'deferred', { record: { reason: 'the diagram lands next sprint', by: '@al', date: '2026-08-01' } }),
    S('stories', 'in_progress'),
  );
  assert.deepEqual(preconditionsMet(mixed, 'stories'), { ok: true, blockedBy: null, reason: 'ready' });

  // A BLOCKED step is the one recorded state that stops the chain, and it is named as the blocker.
  const stuck = chain(
    S('epic', 'done'),
    S('epic-review', 'blocked', { record: REC }),
    S('architecture', 'todo'),
  );
  assert.deepEqual(preconditionsMet(stuck, 'architecture'),
  ...
  assert.match(act.why, /waiting on the payments vendor/);
  ...
  assert.equal(preconditionsMet(weird, 'epic-review').blockedBy, 'epic');
});

test('a blocked step survives a gate write, and loses its record only when it passes', () => {
  const chain = () => ({
    epicId: 'EP-x', currentStep: 'epic-review',
    steps: [
      { id: 'epic', type: 'author', artifact: 'epic.md', status: 'done' },
      { id: 'epic-review', type: 'review+approve', artifact: 'epic.md', status: 'blocked', record: { ...REC } },
      { id: 'architecture', type: 'author', artifact: 'architecture.md', status: 'todo' },
    ],
  });

  // `gate sync` reaches `markInReview` on the path where the gate did NOT pass. Writing `in_review`
  // there would drop the record naming who we wait on and change nothing about the wait.
  const held = chain();
  markInReview(held, byId(held, 'epic-review'));
  assert.equal(byId(held, 'epic-review').status, 'blocked', 'a review does not clear a blocker');
  assert.deepEqual(byId(held, 'epic-review').record, REC, 'and the record naming the blocker survives');

  // When the gate DOES pass, the step is no longer waiting on anybody — so the record goes with the
  // state it belonged to, rather than leaving a `done` step still explaining who it waits for.
  const freed = chain();
  advanceState(freed, byId(freed, 'epic-review'));
  assert.equal(byId(freed, 'epic-review').status, 'done');
  assert.equal(byId(freed, 'epic-review').record, undefined);
  assert.equal(freed.currentStep, 'architecture');

  // A SKIP's record is the opposite case: it is the reason the step passed, so it has to survive.
  const skipped = { epicId: 'EP-y', currentStep: 'ui-design-review', steps: [
    { id: 'ui-design', type: 'author', artifact: 'ui-design.md', status: 'skipped', skipped: true, skipReason: 'no UI', record: { reason: 'no UI', by: null, date: null } },
    { id: 'ui-design-review', type: 'review+approve', artifact: 'ui-design.md', status: 'skipped', skipped: true, skipReason: 'no UI', record: { reason: 'no UI', by: null, date: null } },
  ] };
  advanceState(skipped, byId(skipped, 'ui-design-review'));
  assert.deepEqual(byId(skipped, 'ui-design-review').record, { reason: 'no UI', by: null, date: null });
});

test('advanceState steps over a shape-7 skip that carries no legacy flag', () => {
  const state = {
    epicId: 'EP-x',
    ...
    steps: [
      { id: 'architecture', type: 'author', artifact: 'architecture.md', status: 'done' },
      { id: 'architecture-review', type: 'review+approve', artifact: 'architecture.md', status: 'in_review' },
      // No `skipped: true` beside it — the shape-7 spelling on its own.
      { id: 'ui-design', type: 'author', artifact: 'ui-design.md', status: 'skipped', record: { reason: 'no UI', by: null, date: null } },
      { id: 'ui-design-review', type: 'review+approve', artifact: 'ui-design.md', status: 'skipped', record: { reason: 'no UI', by: null, date: null } },
      { id: 'stories', type: 'author', artifact: 'stories', status: 'todo' },
    ],
  };
  advanceState(state, byId(state, 'architecture-review'));
  assert.equal(state.currentStep, 'stories', 'the skipped pair is stepped over on the new spelling too');
  assert.equal(byId(state, 'stories').status, 'in_progress');
});

test('gatePredicate honours a skip and an inheritance in the shape-7 spelling as well as the legacy one', () => {
  const run = (step, optional = ['ui-design']) => gatePredicate({ step, approvals: [], optional, merged: false, threadsResolved: false });

  // `satisfied` with no `inherited` flag still short-circuits, and keeps the label the golden freezes.
  const sat = run({ id: 'ui-design-review', status: 'satisfied', inheritedFrom: 'EP-parent' });
  assert.equal(sat.passed, true);
  assert.equal(sat.rule, 'inherited', 'the gate label is the frozen word, not the state name');
  assert.equal(sat.have, null, 'nothing was counted here — which is not the same fact as zero approvals');

  const skip = run({ id: 'ui-design-review', status: 'skipped', record: { reason: 'no UI' } });
  assert.equal(skip.passed, true);
  assert.equal(skip.rule, 'skipped');

  // A skip on a step THIS epic's route does not mark optional falls through to the real rule, in
  // either spelling — the route guard is what decides, never the claim.
  const forged = run({ id: 'stories-review', status: 'skipped' }, ['ui-design']);
  assert.equal(forged.passed, false);
  assert.notEqual(forged.rule, 'skipped');

  // `deferred` and `blocked` get NO short-circuit: neither one says the review happened, so the gate
  // asks for its approvals exactly as it would on a step nobody had touched.
  for (const status of ['deferred', 'blocked']) {
    const p = run({ id: 'ui-design-review', status, record: REC });
    assert.equal(p.passed, false, `${status} does not waive a gate`);
    assert.ok(p.missing.length, `${status} still reports what is missing`);
  }
});

test('stampStepStates: shape 7 translates what the file already said, and invents nothing', () => {
  const legacy = {
    epicId: 'EP-x',
    currentStep: 'stories',
    steps: [
      { id: 'epic', type: 'author', artifact: 'epic.md', status: 'done', inherited: true, inheritedFrom: 'EP-parent', boundHash: 'sha256:aa' },
      { id: 'ui-design', type: 'author', artifact: 'ui-design.md', status: 'done', skipped: true, skipReason: 'backend only', skippedBy: '@al', skippedAt: '2026-07-08' },
      { id: 'stories', type: 'author', artifact: 'stories', status: 'in_progress' },
      { id: 'stories-review', type: 'review+approve', artifact: 'stories', status: 'blocked' },
    ],
  };
  const out = stampStepStates(legacy);
  const step = (id) => out.steps.find((s) => s.id === id);

  assert.equal(step('epic').status, 'satisfied');
  assert.deepEqual(step('epic').record, { reason: 'carried by reference from EP-parent', by: null, date: null, link: 'EP-parent' });
  assert.equal(step('epic').inherited, true, 'the legacy flag is KEPT — add before you remove');
  assert.equal(step('epic').boundHash, 'sha256:aa', 'and boundHash stays its own field: it is drift evidence, not provenance');
  ...
  assert.deepEqual(step('ui-design').record, { reason: 'backend only', by: '@al', date: '2026-07-08' },
    'the record is assembled from what the skip already recorded — nothing is invented');
  assert.equal(step('ui-design').skipReason, 'backend only', 'and the four legacy fields are kept');
  ...
  assert.equal(step('stories-review').record, undefined, 'and gains no record — `todo` has nothing to explain');

  // Idempotent, and the SAME object back when there is nothing to move — `writeJSON` short-circuits
  // on identical bytes, and a fresh object every time would defeat that on every read-only path.
  const twice = stampStepStates(out);
  assert.equal(twice, out, 'a second run is a no-op, by identity');
  assert.equal(stampStepStates({ steps: [{ id: 'x', status: 'quantum' }] }).steps[0].status, 'quantum',
    'a status this release cannot name is left exactly as it stands — the file wins');
  // A record the team wrote themselves is never rewritten by an upgrade.
  const mine = stampStepStates({ steps: [{ id: 'x', status: 'done', skipped: true, skipReason: 'a', record: { reason: 'the real reason' } }] });
  assert.deepEqual(mine.steps[0].record, { reason: 'the real reason' });
});

test('seedState writes the model\'s word for "not started"', () => {
  const seeded = seedState({ epic: 'EP-x', profile: 'classic', type: 'feature', today: '2026-08-01' });
  assert.equal(seeded.steps[0].status, 'in_progress', 'the first step opens');
  assert.deepEqual([...new Set(seeded.steps.slice(1).map((s) => s.status))], ['todo'],
    'and every step behind it is `todo`, never the old `blocked`');
  const stub = seedState({ epic: 'EP-y', profile: 'classic', type: 'feature', today: '2026-08-01', stub: true });
  assert.deepEqual([...new Set(stub.steps.map((s) => s.status))], ['todo'], 'a stub anchor seeds a chain nobody is running');
});

test('skipStep: at the ui-design step marks both ui steps N/A (skipped) and advances currentStep to stories', () => {
  ...
  // Shape 7 (E38): the step's own state says what happened, and the four legacy fields stay beside
  // it so a reader on either vocabulary sees the same step.
  assert.equal(ui.status, 'skipped');
  ...
  assert.deepEqual(ui.record, { reason: 'backend-only epic', by: '@me', date: '2026-07-08' });
  assert.equal(uir.skipped, true);
  assert.equal(uir.status, 'skipped');
  assert.ok(isPassed(ui) && isPassed(uir), 'a skipped step is passed — the chain continues past it');
  assert.ok(!isAuthored(ui), 'and it is NOT authored — there is no artifact to audit');
...
test('unskipStep: restores the ui pair and re-points currentStep back to ui-design (before stories authoring)', () => {
  ...
  assert.equal(byId(state, 'ui-design').record, undefined, 'and so is the skip record');
  ...
  assert.equal(byId(state, 'stories').status, 'todo', 'the auto-opened downstream is pushed back behind ui-design');
  assert.equal(state.currentStep, 'ui-design');
});
...
test('undeferStep puts the pair back under the un-skip window, and neither verb converts the other (E37)', () => {
  ...
  // Stories finished while the UI waited. E37 refused here; from E41 the deferral re-opens behind them
  // (the next test follows it through). An un-skip is still refused: for a skip the window is the meaning.
  const built = uiChain('ui-design', { ui: 'in_progress' });
  deferStep(built, 'ui-design', { reason: 'later' });
  byId(built, 'stories').status = 'done';
  undeferStep(built, 'ui-design');
  assert.equal(byId(built, 'ui-design').status, 'in_progress');
  assert.equal(byId(built, 'stories').status, 'done', 'finished work stays finished');
  const skippedBuilt = uiChain('ui-design', { ui: 'in_progress' });
  skipStep(skippedBuilt, 'ui-design', { reason: 'no UI' });
  byId(skippedBuilt, 'stories').status = 'done';
  assert.throws(() => unskipStep(skippedBuilt, 'ui-design'),
    (e) => /stories is already 'done'/.test(e.message) && /the chain has built on the skip since/.test(e.hint));
...
test('undeferStep after later work finished re-opens the pair beside it, and the chain carries it (E41)', () => {
  const state = builtPastDeferral();
  undeferStep(state, 'ui-design');
  assert.equal(byId(state, 'ui-design').status, 'in_progress');
  assert.equal(byId(state, 'ui-design').record, undefined, 'the deferral record goes with the deferral');
  assert.equal(byId(state, 'ui-design-review').status, 'todo');
  assert.equal(byId(state, 'stories').status, 'done');
  assert.equal(byId(state, 'stories-review').status, 'in_review');
  assert.equal(state.currentStep, 'stories-review', 'currentStep stays where the chain is');
  assert.equal(isReopenedStep(state, 'ui-design'), true);
  assert.equal(isReopenedStep(state, 'stories-review'), false);

  // (1) preconditionsMet does not name the re-opened step the blocker of the work in front of it — but
  // its own gate still waits on it.
  assert.deepEqual(preconditionsMet(state, 'stories-review'), { ok: true, blockedBy: null, reason: 'ready' });
  assert.equal(preconditionsMet(state, 'ui-design').ok, true);
  assert.equal(preconditionsMet(state, 'ui-design-review').blockedBy, 'ui-design');

  // (2) The stories review passes: the epic reaches Build with the lane still open.
  ...
  advanceState(state, byId(state, 'ui-design-review'));
...
test('markInReview and advanceState only move currentStep forward, and open only a `todo` step (E41)', () => {
  // The lane's review opens while currentStep sits on a LATER review: it stays there.
  const state = builtPastDeferral();
  undeferStep(state, 'ui-design');
  markInReview(state, byId(state, 'ui-design-review'));
  assert.equal(state.currentStep, 'stories-review');
  // It passes while the stories review is still open: the stories are left done, currentStep left put.
  advanceState(state, byId(state, 'ui-design-review'));
  assert.equal(byId(state, 'stories').status, 'done');
  assert.equal(state.currentStep, 'stories-review');

  // In the ordinary forward case the next step is still opened and currentStep follows it…
  const plain = uiChain('architecture-review', { ui: 'todo', uiReview: 'todo', stories: 'todo' });
...
test('preconditionsMet: only work COMPLETED here after an unfinished step makes it a re-opened lane (E41)', () => {
  const chain = (...statuses) => ({ epicId: 'EP-x', currentStep: 'd',
    steps: statuses.map(([id, status, extra = {}]) => ({ id, type: id.endsWith('-review') ? 'review+approve' : 'author', artifact: `${id}.md`, status, ...extra })) });
  // `done` after the open step: a lane, so the step behind the finished work is ready.
  assert.deepEqual(preconditionsMet(chain(['a', 'done'], ['b', 'in_progress'], ['c', 'done'], ['d', 'todo']), 'd'),
    { ok: true, blockedBy: null, reason: 'ready' });
  // A skipped, deferred or inherited step after it is NOT work built here — the open step still blocks.
  const rec = { record: { reason: 'x' } };
  for (const [status, extra] of [['skipped', rec], ['deferred', rec], ['satisfied', rec], ['done', { inherited: true }], ['done', { skipped: true }]]) {
    assert.equal(preconditionsMet(chain(['a', 'done'], ['b', 'todo'], ['c', status, extra], ['d', 'todo']), 'd').blockedBy, 'b', `${status} ${JSON.stringify(extra)}`);
  }
  // A done GATE over its own unwritten author step is the #131 damage, not a re-open.
...
test('a re-opened review goes through `yad gate open` and `yad gate sync` beside Build, and paying it clears the debt (E41)', async () => {
  ...
  try {
    git(T, 'init', '-q'); git(T, 'config', 'user.email', 'a@b.c'); git(T, 'config', 'user.name', 'x');
    fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
    fs.writeFileSync(path.join(T, '.sdlc/hub.json'), JSON.stringify({ platform: 'github', default_branch: 'main',
      roster: [{ login: 'al', name: 'alice', role: 'owner' }, { login: 'bo', name: 'bob', role: 'reviewer' }] }));
    fs.writeFileSync(path.join(T, '.sdlc/repos.json'), JSON.stringify({ repos: [] }));
    const ep = path.join(T, 'epics/EP-x');
    fs.mkdirSync(path.join(ep, '.sdlc'), { recursive: true });
    ...
    fs.writeFileSync(path.join(ep, '.sdlc/state.json'), JSON.stringify(state));
    const read = () => JSON.parse(fs.readFileSync(path.join(ep, '.sdlc/state.json')));

    const creator = () => ({ ok: true, url: 'http://x/9', number: 9 });
    await grab(() => gateOpen(T, { epic: 'EP-x', artifact: 'ui-design.md', creator, hasBranch: () => true }));
    let after = read();
    ...
    const merged = { ok: true, state: 'merged', merged: true, headOid: 'a', threads: [], reviews: [
      { login: 'al', state: 'APPROVED', submittedAt: '2026-09-14T00:00:00Z' },
      { login: 'bo', state: 'APPROVED', submittedAt: '2026-09-14T00:00:00Z' },
    ] };
    const out = await grab(() => gateSync(T, { epic: 'EP-x', today: '2026-09-14', reader: () => merged }));
    assert.match(out, /gate PASSED — ui-design-review → done; next: ready-for-build/);
    after = read();
...
test('gate sync honours a skip only when the epic\'s route allows it', async () => {
  ...
  // never writes that combination — it always stamps the step skipped — so this is proof the route
  // reaches the predicate, not a claim about what a real ledger looks like. The realistic shape is at
  // the end.
  ...
  try {
    ...
    // `skipped`, not `done`: the gate advanced it, and the shape-7 stamp in `writeState` then names
    // HOW it finished — the step still carries `skipped: true`, so the model's word for it is `skipped`.
    assert.equal(state.steps.find((x) => x.id === 'ui-design-review').status, 'skipped');
  ...
  try {
    ...
    // Still PASSED — nothing is un-advanced, which is the whole observable effect of failing closed
    // on a live project. The word is `skipped` rather than `done` because the shape-7 stamp in
    // `writeState` names how the step finished; `isPassed` is the same answer either way.
    assert.equal(st.steps.find((x) => x.id === 'ui-design-review').status, 'skipped');
    assert.equal(st.currentStep, 'stories', 'and the chain was not rolled back');
...
test('doctor reports an unknown step state and a recorded one with nothing recorded, and corrects neither', async () => {
  const { stepStateChecks, collectDoctor } = await import('./doctor.mjs');
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e38-doc-'));
  const write = (epic, state) => {
    fs.mkdirSync(path.join(T, `epics/${epic}/.sdlc`), { recursive: true });
    const file = path.join(T, `epics/${epic}/.sdlc/state.json`);
    fs.writeFileSync(file, JSON.stringify(state, null, 2));
    return file;
  };
  try {
    const file = write('EP-x', {
      schemaVersion: 7, epicId: 'EP-x', currentStep: 'stories',
      steps: [
        // A word from a newer release. Reported, never rewritten — the file wins for this major.
        { id: 'epic', type: 'author', artifact: 'epic.md', status: 'quantum' },
        // Recorded states with nothing recorded on them.
        { id: 'architecture', type: 'author', artifact: 'architecture.md', status: 'skipped' },
        { id: 'ui-design', type: 'author', artifact: 'ui-design.md', status: 'blocked' },
        // …and the same states WITH a record, which must stay silent.
        { id: 'test-cases', type: 'author', artifact: 'test-cases.md', status: 'deferred', record: { reason: 'next sprint', by: null, date: null } },
        { id: 'stories', type: 'author', artifact: 'stories/', status: 'in_progress' },
      ],
    });
    const before = fs.readFileSync(file, 'utf8');
    ...
    assert.deepEqual(checks.map((x) => x.id).sort(), ['step:no-record', 'step:unknown-status']);
    assert.deepEqual([...new Set(checks.map((x) => x.status))], ['warn'], 'both report, neither corrects');
    const unknown = checks.find((x) => x.id === 'step:unknown-status');
    assert.match(unknown.message, /EP-x\/epic: `quantum`/);
    const noRec = checks.find((x) => x.id === 'step:no-record');
    assert.match(noRec.message, /2 step\(s\)/);
    assert.match(noRec.message, /EP-x\/architecture \(skipped\)/);
    assert.match(noRec.message, /EP-x\/ui-design \(blocked\)/);
    assert.doesNotMatch(noRec.message, /test-cases/, 'a state that DID record why must stay silent');
    assert.equal(fs.readFileSync(file, 'utf8'), before, 'doctor rewrote the ledger');

    // THE TWO NARROWINGS, and each is what keeps the check from reporting a healthy project.
    //
    // 1. The LEGACY encodings carry their own provenance and are not asked for a `record`. Without
    //    this, every change-epic in every pre-shape-7 project — the frozen golden one included —
    //    would be reported for a field it had no way to write.
    write('EP-y', {
      schemaVersion: 7, epicId: 'EP-y', currentStep: 'stories',
      steps: [
        { id: 'epic', type: 'author', artifact: 'epic.md', status: 'done', inherited: true, inheritedFrom: 'EP-p', boundHash: 'sha256:a' },
        { id: 'ui-design', type: 'author', artifact: 'ui-design.md', status: 'done', skipped: true, skipReason: 'no UI', skippedBy: null, skippedAt: null },
      ],
    });
    // 2. A literal `blocked` below shape 7 is the OLD spelling of `todo` and has nothing to explain.
    write('EP-z', {
      schemaVersion: 6, epicId: 'EP-z', currentStep: 'epic',
      steps: [{ id: 'epic', type: 'author', artifact: 'epic.md', status: 'blocked' }],
    });
    const narrowed = [];
    stepStateChecks(narrowed, T);
    for (const c of narrowed) {
      assert.doesNotMatch(c.message, /EP-y/, 'a legacy encoding carries its own provenance');
      assert.doesNotMatch(c.message, /EP-z/, 'a pre-shape-7 `blocked` is the old spelling of `todo`');
    }

    // A project holding only healthy steps says nothing at all, which is the normal state.
    const healthy = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e38-ok-'));
    fs.mkdirSync(path.join(healthy, 'epics/EP-a/.sdlc'), { recursive: true });
    fs.writeFileSync(path.join(healthy, 'epics/EP-a/.sdlc/state.json'), JSON.stringify({
      schemaVersion: 7, epicId: 'EP-a', currentStep: 'epic',
      steps: [{ id: 'epic', type: 'author', artifact: 'epic.md', status: 'in_progress' },
        { id: 'epic-review', type: 'review+approve', artifact: 'epic.md', status: 'todo' }],
    }));
    const quiet = [];
    stepStateChecks(quiet, healthy);
    assert.deepEqual(quiet, []);
    fs.rmSync(healthy, { recursive: true, force: true });

    // …and it is WIRED, not merely exported, and sits inside the `shape` block — the renderer prints
    // a header every time the section changes as it walks the array.
    fs.writeFileSync(file, before);
    fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
    fs.writeFileSync(path.join(T, '.sdlc/cli-version.json'), JSON.stringify({ version: '3.0.0' }));
    const wired = collectDoctor(T).checks.find((c) => c.id === 'step:unknown-status');
    assert.ok(wired, 'stepStateChecks is exported but never called');
    assert.equal(wired.section, 'shape');
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});
...
test('unskipStep: an UPSTREAM skip un-skips back to todo and leaves currentStep put', () => {
  ...
  assert.equal(byId(state, 'ui-design').status, 'todo', 'prior step not done → author returns to not-started');
  assert.equal(byId(state, 'ui-design-review').status, 'todo');
...
test('planMode: solo needs a reason and team does not; the same mode twice changes nothing; the older review_gate.solo goes off with team (E10)', async () => {
  const { planMode, modeFields, modeOf } = await import('./mode.mjs');
  let p = planMode({ solo: false }, { to: 'solo' });
  assert.equal(p.ok, false);
  assert.match(p.message, /needs a reason/);
...
test('yad mode writes both names and the record, names the open reviews, reads back, and never writes over a broken file (E10)', async () => {
  ...
  const read = (f) => JSON.parse(fs.readFileSync(path.join(T, '.sdlc', f), 'utf8'));
  try {
    let r = await run(() => runMode(T, { to: 'solo' }));
    assert.equal(r.failed, true);
    assert.match(r.out, /needs a reason/);
    assert.equal(read('hub.json').solo, undefined, 'a refusal writes nothing');

    r = await run(() => runMode(T, { to: 'solo', reason: 'the team is on leave', today: '2026-09-15' }));
    assert.equal(r.failed, false, r.out);
    for (const f of ['hub.json', 'product.json']) {
      const h = read(f);
      assert.deepEqual([h.solo, h.mode, h.mode_set.from, h.mode_set.to, h.mode_set.reason, h.mode_set.date],
        [true, 'solo', 'team', 'solo', 'the team is on leave', '2026-09-15'], f);
      assert.equal(h.roster.length, 3, `${f} keeps everything else`);
    }
    assert.match(r.out, /1 open review\(s\) follow the solo rule from their next sync: EP-test architecture-review/);
    assert.match(r.out, /reverse with `yad mode team`/);

    r = await run(() => runMode(T, { to: 'solo', reason: 'again' }));
    assert.match(r.out, /already solo — nothing changed/);
    assert.doesNotMatch(r.out, /commit /, 'nothing written, nothing to commit');

    r = await run(() => runMode(T, { json: true }));
    const j = JSON.parse(r.out);
    assert.deepEqual([j.ok, j.mode, j.name, j.agrees, j.set.reason], [true, 'solo', 'solo', true, 'the team is on leave']);

    r = await run(() => runMode(T, { to: 'team', json: true, today: '2026-09-16' }));
...
test('doctor mode:disagree: silent with no `mode` or when it agrees; warns when `solo` says the other (E10)', async () => {
  ...
  try {
    assert.equal(found({ solo: true }), undefined, 'a Product set up before E10 reads exactly as before');
    assert.equal(found({ solo: true, mode: 'solo' }), undefined);
    assert.equal(found({ mode: 'team' }), undefined);
    let c = found({ solo: true, mode: 'team' });
    assert.equal(c.status, 'warn');
    assert.match(c.message, /mode: "team", but solo mode is on/);
    assert.match(c.hint, /`yad mode solo` keeps what the gates do now/);
    assert.equal(found({ review_gate: { solo: true }, mode: 'team' })?.status, 'warn', 'the older spelling counts as solo');
    c = found({ solo: false, mode: 'duo' });
    assert.match(c.hint, /writes a mode the gates recognise/);
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});
...
// ---- E39: a whole Build lane skipped — one story in one repo --------------------------------------
const laneRecord = { reason: 'no UI change', by: 'sam', date: '2026-09-14' };
function laneEpic(T, { storiesReview = 'done', stories = { 'EP-l-S01': ['api', 'web'] }, storyStatus = 'approved', buildState = {}, ships = [] } = {}) {
  const dir = path.join(T, 'epics/EP-l');
  fs.mkdirSync(path.join(dir, '.sdlc/build-state'), { recursive: true });
  fs.mkdirSync(path.join(dir, 'stories'), { recursive: true });
  const st = (id, type, status) => ({ id, type, artifact: id.startsWith('stories') ? 'stories/' : 'epic.md', status, risk_tags: [] });
  fs.writeFileSync(path.join(dir, '.sdlc/state.json'), JSON.stringify({
    epicId: 'EP-l', profile: 'chore', currentStep: storiesReview === 'done' ? 'ready-for-build' : 'stories-review',
    steps: [st('epic', 'author', 'done'), st('epic-review', 'review+approve', 'done'), st('stories', 'author', 'done'), st('stories-review', 'review+approve', storiesReview)],
  }));
  fs.writeFileSync(path.join(dir, '.sdlc/approvals.json'), '[]');
  fs.writeFileSync(path.join(dir, '.sdlc/comments.json'), '[]');
  for (const [id, repos] of Object.entries(stories)) {
    fs.writeFileSync(path.join(dir, `stories/${id}.md`), `---\nid: ${id}\nepic: EP-l\nstatus: ${storyStatus}\nrepos: [${repos.join(', ')}]\n---\n`);
  }
  for (const [story, bs] of Object.entries(buildState)) fs.writeFileSync(path.join(dir, `.sdlc/build-state/${story}.json`), JSON.stringify(bs));
  if (ships.length) {
    fs.mkdirSync(path.join(dir, '.sdlc/build-log'), { recursive: true });
    ships.forEach((sh, i) => fs.writeFileSync(path.join(dir, `.sdlc/build-log/${sh.story}-t${i}-${sh.repo}.json`), JSON.stringify({ task: `t${i}`, ...sh })));
  }
  return dir;
}

test('skipLane / unskipLane: a whole lane, with every refusal the E39 decisions call for (E39)', async () => {
  const { skipLane, unskipLane } = await import('./epic-state.mjs');
  const base = { story: 'EP-l-S01', repo: 'web', reason: 'no UI change', by: 'sam', date: '2026-09-14', declared: ['api', 'web'], storiesPassed: true };
  const refusal = (fn) => { try { fn(); } catch (e) { assert.equal(e.code, 'YAD-STATE-004'); return e.message; } assert.fail('expected a refusal'); };

  const { buildState, already } = skipLane(null, base);
  assert.equal(already, false);
  assert.deepEqual(buildState, { story: 'EP-l-S01', repos: { web: { status: 'skipped', record: laneRecord } } });

  assert.match(refusal(() => skipLane(null, { ...base, storiesPassed: false })), /stories review has not passed/);
  assert.match(refusal(() => skipLane(null, { ...base, repo: 'Web' })), /does not declare Web/, 'names are case-sensitive');
  assert.match(refusal(() => skipLane(null, { ...base, shippedRepos: ['web'] })), /already has a ship recorded in web/);
  assert.match(refusal(() => skipLane({ story: 'EP-l-S01', repos: { web: { steps: [{ id: 'spec', status: 'in_progress' }] } } }, base)), /work has started/);
  assert.match(refusal(() => skipLane({ story: 'EP-l-S01', repos: { web: { steps: [{ id: 'spec', status: 'blocked', record: { reason: 'vendor' } }] } } }, base)),
    /work has started/, 'a halt is work started');
  assert.match(refusal(() => skipLane(skipLane(null, { ...base, repo: 'api' }).buildState, base)), /web is the last lane EP-l-S01 has left/);
  assert.match(refusal(() => skipLane(null, { ...base, declared: ['web'] })), /last lane/);
  assert.match(refusal(() => skipLane(null, { ...base, reason: '  ' })), /needs a reason/);
  assert.match(refusal(() => skipLane(null, { ...base, reason: true })), /needs a reason/);

  // A lane the skill seeded but never began is replaced; everything else in the file is kept.
  const untouched = { story: 'EP-l-S01', note: 'kept', repos: { web: { currentStep: 'spec', steps: [{ id: 'spec', status: 'todo' }] }, api: { steps: [] } } };
  const replaced = skipLane(untouched, base).buildState;
  assert.equal(replaced.note, 'kept');
  assert.deepEqual(replaced.repos.api, { steps: [] });
  assert.deepEqual(replaced.repos.web, { status: 'skipped', record: laneRecord });

  const again = skipLane(buildState, { ...base, reason: undefined, by: 'kim' });
  assert.equal(again.already, true, 'a repeat is not refused for lacking a fresh reason');
  assert.equal(again.buildState.repos.web.record.by, 'sam', 'and keeps the original record');

  assert.deepEqual(unskipLane(buildState, { story: 'EP-l-S01', repo: 'web' }), { buildState: { story: 'EP-l-S01', repos: {} }, empty: true });
  const partial = unskipLane(replaced, { story: 'EP-l-S01', repo: 'web' });
  assert.equal(partial.empty, false, 'another lane and another key stay');
  assert.deepEqual(Object.keys(partial.buildState.repos), ['api']);
  assert.match(refusal(() => unskipLane(untouched, { story: 'EP-l-S01', repo: 'web' })), /is not skipped/);
  assert.match(refusal(() => unskipLane(null, { story: 'EP-l-S01', repo: 'web' })), /is not skipped/);
});

test('a skipped lane is owed nothing: buildNextForRepo, the Build line, and featureStatus read it that way (E39)', async () => {
  const { buildNextForRepo, nextAction, featureStatus } = await import('./epic-state.mjs');
  assert.deepEqual(buildNextForRepo({ status: 'skipped', record: laneRecord }),
    { step: null, status: 'skipped', shipped: false, skill: null, automation: null, locked: false, chain: [], record: laneRecord });
  assert.equal('record' in buildNextForRepo({ status: 'skipped' }), false, 'no record key unless there is a record');
  assert.equal('record' in buildNextForRepo({ steps: [{ id: 'spec', status: 'done' }] }), false, 'an ordinary lane gains no key');
  ...
  assert.equal(featureStatus(L({ api: done, web: { status: 'skipped', record: laneRecord } }), { stories }), 'shipped');
  assert.equal(featureStatus(L({ api: { status: 'skipped' }, web: { status: 'skipped' } }), { stories }), 'in-build', 'every lane skipped shipped nothing');
  assert.equal(featureStatus(L({ api: done }), { stories }), 'in-build', 'web has no lane, so it is still owed');
});

test('yad skip <epic> <story> --repo: writes the lane, refuses what E39 refuses, and unskip takes it back out (E39)', async () => {
  const { runLaneSkip } = await import('./skip.mjs');
  const { runNext } = await import('./next.mjs');
  const prev = process.exitCode;
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-lane-skip-'));
  try {
    const dir = laneEpic(T);
    const file = path.join(dir, '.sdlc/build-state/EP-l-S01.json');
    const out = await grab(() => runLaneSkip(T, { epic: 'EP-l', story: 'EP-l-S01', repo: 'web', reason: 'no UI change', today: '2026-09-14' }));
    assert.match(out, /EP-l-S01 \/ web lane skipped — N\/A/);
    assert.match(out, /reason: no UI change/);
    assert.match(out, /yad checkpoint --push/);
    assert.match(out, /yad unskip EP-l EP-l-S01 --repo web/);
    const bs = JSON.parse(fs.readFileSync(file, 'utf8'));
    assert.equal(bs.repos.web.status, 'skipped');
    assert.deepEqual([bs.repos.web.record.reason, bs.repos.web.record.date], ['no UI change', '2026-09-14']);
    assert.match(await grab(() => runLaneSkip(T, { epic: 'EP-l', story: 'EP-l-S01', repo: 'web', today: '2026-09-15' })), /was already skipped.*nothing changed/);
    await assert.rejects(() => runLaneSkip(T, { epic: 'EP-l', story: 'EP-l-S01', repo: 'api', reason: 'x', today: '2026-09-14' }), /last lane/);
    assert.match(await grab(() => runNext(T, { epic: 'EP-l' })), /EP-l-S01 \/ web — skipped \(N\/A\): no UI change/);

    const un = await grab(() => runLaneSkip(T, { epic: 'EP-l', story: 'EP-l-S01', repo: 'web', undo: true, reason: 'x' }));
    assert.match(un, /EP-l-S01 \/ web un-skipped/);
    assert.match(un, /--reason is not used when un-skipping/);
    assert.equal(fs.existsSync(file), false, 'the file the skip created goes with it');

    process.exitCode = undefined;
    assert.match(await grab(() => runLaneSkip(T, { epic: 'EP-l', story: 'EP-l-S09', repo: 'web', reason: 'x' })), /no story EP-l-S09/);
    assert.equal(process.exitCode, 1);
    process.exitCode = undefined;
    assert.match(await grab(() => runLaneSkip(T, { epic: 'EP-l', story: 'EP-l-S01', reason: 'x' })), /usage: yad skip EP-l EP-l-S01 --repo <name>/);
    assert.equal(process.exitCode, 1);
    process.exitCode = undefined;

    // The CLI routes a story id to the lane, and refuses a lane deferral.
    const cli = yadRun(T, 'skip', 'EP-l', 'EP-l-S01', '--repo', 'web', '--reason', 'no UI change');
    assert.equal(cli.code, 0, cli.out);
    const deferred = yadRun(T, 'defer', 'EP-l', 'EP-l-S01', '--repo', 'web', '--reason', 'x');
    assert.equal(deferred.code, 1);
    assert.match(deferred.out, /skipped whole, never deferred/);
    assert.equal(yadRun(T, 'unskip', 'EP-l', 'EP-l-S01', '--repo', 'web').code, 0);
    assert.match(yadRun(T, '--help').out, /yad skip <epic> <story> --repo <name>/);
  } finally { process.exitCode = prev; fs.rmSync(T, { recursive: true, force: true }); }

  const early = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-lane-skip-early-'));
  try {
    laneEpic(early, { storiesReview: 'in_review' });
    await assert.rejects(() => runLaneSkip(early, { epic: 'EP-l', story: 'EP-l-S01', repo: 'web', reason: 'x' }), /stories review has not passed/);
  } finally { fs.rmSync(early, { recursive: true, force: true }); }

  const shipped = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-lane-skip-shipped-'));
  try {
    laneEpic(shipped, { ships: [{ story: 'EP-l-S01', repo: 'web' }] });
    await assert.rejects(() => runLaneSkip(shipped, { epic: 'EP-l', story: 'EP-l-S01', repo: 'web', reason: 'x' }), /already has a ship recorded in web/);
  } finally { fs.rmSync(shipped, { recursive: true, force: true }); }
});

test('doctor: laneChecks fails a contradiction, and warns about no record, an undeclared repo and one Build step set aside (E39)', async () => {
  const { laneChecks, collectDoctor } = await import('./doctor.mjs');
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-lane-doctor-'));
  try {
    laneEpic(T, {
      stories: { 'EP-l-S01': ['api', 'web'], 'EP-l-S02': ['api', 'web'] },
      buildState: {
        'EP-l-S01': { story: 'EP-l-S01', repos: {
          web: { status: 'skipped', record: laneRecord },
          api: { currentStep: 'tasks', steps: [{ id: 'spec', status: 'skipped' }, { id: 'tasks', status: 'todo' }] },
          mobile: { status: 'skipped' },
        } },
        'EP-l-S02': { story: 'EP-l-S02', repos: {
          api: { status: 'skipped', record: laneRecord, steps: [{ id: 'spec', status: 'done' }] },
          web: { status: 'skipped', record: laneRecord },
        } },
      },
      ships: [{ story: 'EP-l-S02', repo: 'web' }],
    });
    const checks = [];
    laneChecks(checks, T);
    const by = Object.fromEntries(checks.map((x) => [x.id, x]));
    assert.deepEqual(Object.keys(by).sort(), [
      'lane:EP-l:EP-l-S01:api:step-set-aside',
      'lane:EP-l:EP-l-S01:mobile:no-record',
      'lane:EP-l:EP-l-S01:mobile:undeclared',
      'lane:EP-l:EP-l-S02:api:contradiction',
      'lane:EP-l:EP-l-S02:web:contradiction',
    ], 'the healthy skipped lane (S01 web) says nothing');
    assert.equal(by['lane:EP-l:EP-l-S02:api:contradiction'].status, 'fail');
    assert.match(by['lane:EP-l:EP-l-S02:api:contradiction'].message, /work has started in it/);
    assert.match(by['lane:EP-l:EP-l-S02:web:contradiction'].message, /a ship is recorded for it/);
    assert.equal(by['lane:EP-l:EP-l-S01:api:step-set-aside'].status, 'warn');
    assert.match(by['lane:EP-l:EP-l-S01:api:step-set-aside'].message, /spec is skipped — no single Build step may be set aside/);
    assert.equal(by['lane:EP-l:EP-l-S01:mobile:no-record'].status, 'warn');
    assert.equal(by['lane:EP-l:EP-l-S01:mobile:undeclared'].status, 'warn');
    assert.ok(collectDoctor(T).checks.some((x) => x.id.startsWith('lane:')), 'wired into yad doctor, not just exported');
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});

test('checkpoint --retro-ship refuses a skipped lane, and never lists one as still unrecorded (E39)', async () => {
  const { recordRetroShip } = await import('./checkpoint.mjs');
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-lane-retro-'));
  try {
    laneEpic(T, {
      stories: { 'EP-l-S01': ['api', 'web', 'ios'] }, storyStatus: 'in-build',
      buildState: { 'EP-l-S01': { story: 'EP-l-S01', repos: { web: { status: 'skipped', record: laneRecord } } } },
      ships: [{ story: 'EP-l-S01', repo: 'api' }],
    });
    const refused = grabSync(() => recordRetroShip(T, { epic: 'EP-l', story: 'EP-l-S01', repo: 'web', today: '2026-09-14' }));
    assert.match(refused, /EP-l-S01 \/ web is skipped — a lane that was skipped did not ship/);
    assert.match(refused, /yad unskip EP-l EP-l-S01 --repo web/);
    const again = grabSync(() => recordRetroShip(T, { epic: 'EP-l', story: 'EP-l-S01', repo: 'api', today: '2026-09-14' }));
    assert.match(again, /already has a build-log ship in api/);
    assert.match(again, /still unrecorded for EP-l-S01: ios —/, 'web is skipped, so it is owed nothing');
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});
...
test('yad-run and yad-status name the skipped Build lane, so neither drives nor hides one (E39)', () => {
  ...
  assert.match(run, /do not drive\s+it/);
  assert.match(run, /no entry for this `repo`/);
...
// A Product repo with a bare origin: main carries the epic scaffolding (NO hub-prs.json — Path B never
...
// issue #129: a code repo may be a SIBLING of the Product. Wherever only the Product is checked out (Product CI, a
...
test('doctor: an absent repo path OUTSIDE the project root warns instead of failing (#129)', async () => {
  ...
  assert.match(chk.message, /sibling repo, outside the Product/);
...
test('gate loadProduct: an existing hub.json holding literal null is rejected (not treated as absent)', async () => {
  const { T, ep } = scaffoldEpic();
...
test('usage: deriveEvents degrades to [] when the Product is not a git repo', () => {
...
// --- #142: `yad checkpoint --retro-ship` reconciles a PRE-TRACKING shipped story ---

test('runCheckpoint --retro-ship records the missing ship AND carries the flip in one commit (#142)', async () => {
  const prev = process.exitCode;
  const T = productForCheckpoint();
  // A pre-tracking story: committed at `approved` with NO build-log ship (implemented before tracking).
  writeStory(T, 'EP-a', 'EP-a-S01', 'approved', { ship: false });
  git(T, 'add', '-A'); git(T, 'commit', '-q', '-m', 'seed pre-tracking story');
  // The human flips it to shipped in the working tree — but plain checkpoint can't carry it (no ship).
  fs.writeFileSync(path.join(T, 'epics/EP-a/stories/EP-a-S01.md'), `---\nstatus: shipped\nrepos: [web]\n---\n\n# EP-a-S01\n`);
  await grab(() => runCheckpoint(T, { retroShip: { epic: 'EP-a', story: 'EP-a-S01', repo: 'web', today: '2026-07-14' } }));
  const files = git(T, 'diff-tree', '--no-commit-id', '--name-only', '-r', 'HEAD').toString().trim().split('\n').filter(Boolean);
  assert.ok(files.includes('epics/EP-a/.sdlc/build-log/EP-a-S01-retro-web.json'), 'the retroactive ship shard is committed');
  assert.ok(files.includes('epics/EP-a/stories/EP-a-S01.md'), 'the status flip now rides along');
  assert.equal(JSON.parse(fs.readFileSync(path.join(T, 'epics/EP-a/.sdlc/build-log/EP-a-S01-retro-web.json'), 'utf8')).retroactive, true);
  assert.equal(git(T, 'status', '--porcelain').toString().trim(), '', 'nothing left uncommitted — no raw git-to-main needed');
  process.exitCode = prev;
  fs.rmSync(T, { recursive: true, force: true });
});
...
test('runCheckpoint --retro-ship refuses a story that already has a real build-log ship in that repo (#142, #166)', async () => {
  ...
  assert.match(out, /already has a build-log ship/);
  assert.ok(process.exitCode, 'refusal sets a non-zero exit code');
  assert.equal(git(T, 'rev-parse', 'HEAD').toString().trim(), before, 'nothing committed on refusal');
  assert.ok(!fs.existsSync(path.join(T, 'epics/EP-a/.sdlc/build-log/EP-a-S01-retro-web.json')), 'no retro shard written');
  process.exitCode = prev;
  fs.rmSync(T, { recursive: true, force: true });
});
...
test('runCheckpoint --retro-ship REFUSES (no shard, no commit) when frontmatter is not Build — evidence + flip stay atomic (#142)', async () => {
  ...
  const T = productForCheckpoint();
  writeStory(T, 'EP-a', 'EP-a-S01', 'approved', { ship: false });
  git(T, 'add', '-A'); git(T, 'commit', '-q', '-m', 'seed'); // working tree still at `approved` — human forgot to flip
  const before = git(T, 'rev-parse', 'HEAD').toString().trim();
  const out = await grab(() => runCheckpoint(T, { retroShip: { epic: 'EP-a', story: 'EP-a-S01', repo: 'web', today: '2026-07-14' } }));
  assert.match(out, /not at in-build\|shipped/, 'refuses and tells the human to set status: shipped first');
  assert.ok(process.exitCode, 'refusal sets a non-zero exit code');
  assert.equal(git(T, 'rev-parse', 'HEAD').toString().trim(), before, 'nothing committed — no lone drift-creating ship');
  assert.ok(!fs.existsSync(path.join(T, 'epics/EP-a/.sdlc/build-log/EP-a-S01-retro-web.json')), 'no shard written when the flip cannot ride with it');
  process.exitCode = prev;
  fs.rmSync(T, { recursive: true, force: true });
});

test('CLI --retro-ship rejects a path-traversal / malformed / cross-epic story id at the arg boundary (#142)', () => {
  const T = productForCheckpoint();
  // story is always split('/')[1] — it can never contain a slash — so the pin only has to reject a bare
  // `..`, an encoded traversal, a wrong shape, or a cross-epic id. (A multi-slash value like
  // `EP-a/EP-a-S01/../x` reduces to the clean story `EP-a-S01`, so traversal is structurally impossible.)
  for (const bad of ['EP-a/..', 'EP-a/..%2f..%2fetc', 'EP-a/not-a-story', 'EP-a/EP-b-S01']) {
    const r = yadRun(T, 'checkpoint', '--retro-ship', bad, '--repo', 'web', '--dir', T);
    assert.equal(r.code, 1, `rejected: ${bad}`);
    assert.match(r.out, /invalid --retro-ship/, `clear error for: ${bad}`);
  }
  // The guards never touched the filesystem — no epics/ tree was created by a rejected traversal.
  assert.ok(!fs.existsSync(path.join(T, 'epics')), 'a rejected --retro-ship writes nothing');
  fs.rmSync(T, { recursive: true, force: true });
});

test('runCheckpoint --retro-ship --dry-run leaves NO shard on disk and commits nothing (#142)', async () => {
  const prev = process.exitCode;
  const T = productForCheckpoint();
  writeStory(T, 'EP-a', 'EP-a-S01', 'approved', { ship: false });
...
test('recordRetroShip: guards on missing epic/story/repo and a nonexistent target (#142)', () => {
  const prev = process.exitCode;
  const T = productForCheckpoint();
  assert.equal(recordRetroShip(T, { epic: 'EP-a', story: 'EP-a-S01', repo: 'web' }).ok, false, 'a nonexistent epic is rejected');
  fs.mkdirSync(path.join(T, 'epics', 'EP-a', 'stories'), { recursive: true }); // epic exists, story does not
...
test('retroShipRepos: story frontmatter wins, the registry is the fallback, neither blocks a bare legacy story (#166)', () => {
  ...
  // No `repos:` on the story ⇒ the Product's connected repos.
  fs.writeFileSync(storyFile, `---\nstatus: shipped\n---\n\n# EP-a-S01\n`);
...
test('stagedStoryIsStatusOnly: true for a lone status flip, false for a prose edit or a new file', () => {
  ...
  writeStory(T, 'EP-a', 'EP-a-S01', 'approved');
  git(T, 'add', '-A'); git(T, 'commit', '-q', '-m', 'seed');
  const g = productGit(T); // the production git accessor: returns { ok, stdout, stderr, code }
  // lone status flip
...
test('runCheckpoint --allow-branch --push pushes the CURRENT branch, never the default branch (HIGH-1)', async () => {
  const prev = process.exitCode;
  const T = productForCheckpoint(); // default_branch main, on main
...
test('buildCheckpointMessage subject matches the Product commit-message gate regex and collapses injection (NIT-8)', () => {
...
test('writeRetroShip: writes a retroactive shard for a pre-tracking story; refuses when a ship exists (#142)', () => {
  const { T, epicDir } = ledgerEpic();
  // Pre-tracking: no ship at all → writes a retro shard with the sentinel task + retroactive flag.
...
test('writeRetroShip: records mergeCommit + a custom task when supplied; refuses next to a REAL ship (#142)', () => {
  const { T, epicDir } = ledgerEpic();
...
function scaffoldWithRemotes() {
  ...
  git(backend, 'config', 'user.name', 'x');
  return { T, backend, productBare, beBare };
}

test('reconcile --push: commits chore(yad-update) on the Product + connected repo and pushes to origin/main', async () => {
  const prev = process.exitCode;
  ...
  try {
    ...
    process.exitCode = prev;
    for (const d of [T, backend, productBare, beBare]) fs.rmSync(d, { recursive: true, force: true });
  }
});
...
test('commitAndPush: a path that is NOT its own repo top (nested plain dir) is skipped, never committed into the enclosing repo', async () => {
  ...
  try {
    ...
    // a registered repo whose clone is GONE: just a plain dir inside the Product, with recreated wiring
    ...
    const out = await grab(() => {
      const r = commitAndPush({ root: fake, paths: ['checks/spec-link.sh'], items: ['backend/x'] },
        { push: false, productRoot: T, defaultBranch: 'main' });
...
// ---------------------------------------------------------------------------------------------
// `yad repo refresh --push` — publish the connected-repo code-context to the Product default branch
// ---------------------------------------------------------------------------------------------
...
// #126 — the pack strand: setup/refresh must gitignore the pack, and publish must self-heal a Product that
// tracked it before that ignore existed.
test('ensurePackIgnored: idempotently adds the pack glob to the Product .gitignore', () => {
  const { T } = scaffold();
  try {
...
// unrelated edit to the Product `.gitignore` (which a naive `git add .gitignore` would combine with the
...
test('publishCodeContext: never sweeps unrelated work and leaves the user index staged (checkpoint-consistent)', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(git(productBare, 'show', '--name-only', '--format=', 'main').toString(), /\.gitignore/, 'nothing about the user edit reached origin');
    assert.ok(!process.exitCode, 'clean run -> zero exit code');
  } finally {
    process.exitCode = prev;
    for (const d of [T, backend, productBare, beBare]) fs.rmSync(d, { recursive: true, force: true });
...
test('publishCodeContext: commits chore(hub) code-context, pushes to origin/main, then is idempotent', async () => {
  ...
  try {
    process.exitCode = 0;
    // a freshly (AI-)regenerated code-map in the Product working tree
    fs.mkdirSync(path.join(T, '.sdlc/code-context/backend'), { recursive: true });
    fs.writeFileSync(path.join(T, '.sdlc/code-context/backend/code-map.md'), '# backend code-map\n');
    await grab(() => publishCodeContext(T, { push: true }));
...
test('publishCodeContext: a named refresh publishes only that repo\'s code-map, not others', async () => {
  ...
  try {
    process.exitCode = 0;
    // register a second repo and give both an on-disk (uncommitted) code-map in the Product
...
test('publishCodeContext: a re-run pushes a commit that a prior run committed but failed to push', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(git(productBare, 'log', '--oneline', '--all').toString(), /sync code-context/, 'not yet on origin');
    ...
    assert.match(git(productBare, 'log', '-1', '--format=%s', 'main').toString(), /sync code-context/, 'existing commit reaches origin/main on the re-run');
...
// git, faked. `seeded` are the epics whose state.json the base ref carries. Every call is RECORDED,
// so a test can assert the exact ref the probe used — the review found two bugs (a local branch
// instead of origin/, and a repo-root-relative rev:path spec) that a double matching on the path
// half alone made invisible.
function fakeGit(seeded = ['EP-seeded'], { refs = ['origin/main'], lsTreeOk = true } = {}) {
  const calls = [];
  const runner = (cmd, args = []) => {
    calls.push([cmd, ...args].join(' '));
    const line = args.join(' ');
    if (cmd !== 'git') return { ok: false, stdout: '', code: 1 };
    if (line.includes('symbolic-ref')) {
      const head = refs.find((r) => r.startsWith('origin/'));
      return head ? { ok: true, stdout: head, code: 0 } : { ok: false, stdout: '', code: 1 };
    }
    if (line.includes('rev-parse --verify')) {
      const ref = args[args.length - 1].replace(/\^\{commit\}$/, '');
      return { ok: refs.includes(ref), stdout: '', code: 0 };
    }
    if (line.includes('ls-tree')) {
      if (!lsTreeOk) return { ok: false, stdout: '', code: 1 };
      return { ok: true, code: 0, stdout: seeded.map((e) => `epics/${e}/.sdlc/state.json`).join('\0') };
    }
...
test('ledger hook falls back through the gate\'s own base order', () => {
  ...
  try {
    ...
    assert.equal(resolveHookBase(T, { }, r1), 'origin/develop');
    // ...then origin/main...
    const r2 = fakeGit(['EP-seeded'], { refs: ['origin/main'] });
    assert.equal(resolveHookBase(T, {}, r2), 'origin/main');
    // ...and null when no origin ref resolves at all, which ALLOWS (see below).
    const r3 = fakeGit(['EP-seeded'], { refs: [] });
    assert.equal(resolveHookBase(T, {}, r3), null);
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});
...
test('ledger hook allows a NEW epic seed — creation is not mutation (#162)', () => {
  ...
  try {
    ...
    // The whole seed is exempt, not just its first file — a fresh epic's state.json existing in the
    // WORKING TREE must never stand in for the base ref, or every later seed write would be denied.
    for (const rel of ['epics/EP-fresh/.sdlc/approvals.json', 'epics/EP-fresh/.sdlc/comments.json']) {
      assert.equal(decide(T, rel).allow, true, rel);
    }
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});

test('ledger hook case-folds the epic slug, as the CI gate does', () => {
  // On a case-insensitive filesystem `epics/ep-x/…` and `epics/EP-X/…` are the SAME file, so a
  // byte-exact compare lets a mutation be laundered as a creation — the gate folds for this reason.
  const T = hookProduct();
  try {
    const runner = fakeGit(['EP-Seeded']);
    assert.equal(decide(T, 'epics/ep-seeded/.sdlc/state.json', { runner }).allow, false);
    assert.equal(decide(T, 'epics/EP-SEEDED/.sdlc/state.json', { runner: fakeGit(['ep-seeded']) }).allow, false);
    assert.deepEqual([...seededSlugs(T, {}, fakeGit(['EP-MiXeD']))], ['ep-mixed'], 'folded on the way in');
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});

test('ledger hook allows when the base cannot be read — unknown never blocks', () => {
  const T = hookProduct();
  try {
    // No origin ref resolves at all...
    assert.equal(decide(T, 'epics/EP-seeded/.sdlc/state.json', { runner: fakeGit(['EP-seeded'], { refs: [] }) }).allow, true);
    // ...and a base that resolves but cannot be listed.
    assert.equal(decide(T, 'epics/EP-seeded/.sdlc/state.json', { runner: fakeGit(['EP-seeded'], { lsTreeOk: false }) }).allow, true);
    assert.equal(seededSlugs(T, {}, fakeGit([], { refs: [] })), null);
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});
...
test('ledger hook resolves the Product from the edited PATH, not the session root', () => {
  // The documented layout puts code repos BESIDE the Product, so a session opened at the workspace has
...
test('protectedLedgerPath matches the CI gate\'s scope, including its glob depth', () => {
  ...
  // The gate's arms are bash `case` globs and a glob's `*` spans `/`, so it rejects these too —
  // being stricter here would let a path through locally that CI blocks.
  assert.equal(protectedLedgerPath('epics/EP-a/nested/.sdlc/state.json').epic, 'EP-a');
  assert.equal(protectedLedgerPath('epics/EP-a/sub/reviews/x.md').epic, 'EP-a');
  for (const rel of [
    'epics/EP-a/.sdlc/contract-lock.json', 'epics/EP-a/state.json',
    'other/EP-a/.sdlc/state.json', 'epics/EP-a/reviews/x.json', 'epics',
...
// The documented contract end-to-end, through the real binary: stdin payload in, exit code out.
...
test('yad hook ledger-guard honours its stdin/exit-code contract', () => {
  ...
  try {
    ...
    git(T, 'config', 'user.name', 'a');
    fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
    fs.writeFileSync(path.join(T, '.sdlc/hub.json'), JSON.stringify({ platform: 'gitlab', bridge_enabled: true, default_branch: 'main' }));
    fs.mkdirSync(path.join(T, 'epics/EP-a/.sdlc'), { recursive: true });
    fs.writeFileSync(path.join(T, 'epics/EP-a/.sdlc/state.json'), '{}');
    fs.writeFileSync(path.join(T, 'epics/EP-a/epic.md'), '# a');
    git(T, 'add', '-A');
    git(T, 'commit', '-q', '-m', 'seed');
    git(T, 'branch', '-q', '-M', 'main');
    ...
    git(T, 'push', '-q', 'origin', 'main');
    git(T, 'symbolic-ref', 'refs/remotes/origin/HEAD', 'refs/remotes/origin/main');

    const denied = runHook('{"tool_input":{"file_path":"epics/EP-a/.sdlc/state.json"}}');
    assert.equal(denied.code, 2, 'a seeded ledger is denied with exit 2');
    assert.match(denied.stderr, /Blocked/);
    assert.match(denied.stderr, /yad gate open EP-a/);
    assert.doesNotMatch(denied.stderr, /update available/, 'no update banner in front of a model');

    assert.equal(runHook('{"tool_input":{"file_path":"epics/EP-a/epic.md"}}').code, 0, 'an artifact is allowed');
    assert.equal(runHook('{"tool_input":{"file_path":"epics/EP-new/.sdlc/state.json"}}').code, 0, 'a new seed is allowed');
    // Absent, empty and unparseable payloads all allow — the documented fail-open contract.
    for (const bad of ['', '   ', 'not json', '{"tool_input":{}}']) {
      assert.equal(runHook(bad).code, 0, JSON.stringify(bad));
    }
...
test('mergeHookSettings adds our entry once and never touches anything else', () => {
  ...
  // A command WE wrote in an older release is normalised in place, not duplicated.
  const stale = JSON.parse(JSON.stringify(first.settings));
  stale.hooks.PreToolUse[1].hooks[0].command = HOOK_COMMAND_LEGACY[0];
  const fixed = mergeHookSettings(stale);
  assert.equal(fixed.changed, true);
  assert.equal(fixed.settings.hooks.PreToolUse.length, 2);
  assert.equal(fixed.settings.hooks.PreToolUse[1].hooks[0].command, HOOK_COMMAND);
  // But a command we never wrote is the TEAM's, even when it names a similar path: claim it and
  // their own hook silently stops running, on a path that takes no backup.
  const theirs = JSON.parse(JSON.stringify(foreign));
  theirs.hooks.PreToolUse[0].hooks[0].command = 'bash .claude/hooks/ledger-guard.sh';
  const alongside = mergeHookSettings(theirs);
  assert.equal(alongside.settings.hooks.PreToolUse[0].hooks[0].command, 'bash .claude/hooks/ledger-guard.sh');
  assert.equal(alongside.settings.hooks.PreToolUse.length, 2, 'ours is added beside theirs');
...
test('hookMatcherFires tells an armed entry from an installed-but-dead one', () => {
  const armed = mergeHookSettings({}).settings;
  assert.equal(hookMatcherFires(armed), true);
  const narrowed = JSON.parse(JSON.stringify(armed));
  narrowed.hooks.PreToolUse[0].matcher = 'Write';
  assert.equal(hookMatcherFires(narrowed), true, 'still selects a file-editing tool');
  for (const dead of ['Bash', 'Task|WebFetch', '(']) {
    const off = JSON.parse(JSON.stringify(armed));
    off.hooks.PreToolUse[0].matcher = dead;
    assert.equal(hookMatcherFires(off), false, dead);
  }
  const blank = JSON.parse(JSON.stringify(armed));
  blank.hooks.PreToolUse[0].matcher = '';
  assert.equal(hookMatcherFires(blank), true, 'an empty matcher matches every tool');
  assert.equal(hookMatcherFires(null), false);
...
test('hookActions never rewrites a settings file it cannot read — not even with --overwrite-local', () => {
  const T = hookProduct();
  try {
    fs.mkdirSync(path.join(T, '.claude'), { recursive: true });
    const theirs = '{ "permissions": { "allow": ["Bash(ls:*)"] }, }'; // one trailing comma
    fs.writeFileSync(path.join(T, '.claude/settings.json'), theirs);
    const action = hookActions(T, ['.claude']).find((a) => a.item === 'settings.json');
    assert.equal(action.status, 'modified');
    // `--overwrite-local` restores a shipped TEMPLATE; here there is none, and everything the file
    // holds is the team's. Synthesizing a replacement would discard their whole harness config on a
    // generic recovery command run for an unrelated drifted gate script.
    action.apply();
    assert.equal(fs.readFileSync(path.join(T, '.claude/settings.json'), 'utf8'), theirs, 'left untouched');
    // The same refusal at apply() time, when the file breaks between plan and apply.
    fs.writeFileSync(path.join(T, '.claude/settings.json'), '{"permissions":{}}');
    const planned = hookActions(T, ['.claude']).find((a) => a.item === 'settings.json');
    assert.equal(planned.status, 'outdated');
    fs.writeFileSync(path.join(T, '.claude/settings.json'), theirs);
    planned.apply();
...
test('E34 dial rules: a gate is human, the kill switch holds, a Shape dial is project-wide, a lane dial is the lane\'s', async () => {
  const { effectiveAdvance, normalizeAutomation, planShapeDial, planLaneDial, planKill, killSwitchOn, serializeAutomation } = await import('./epic-state.mjs');
  const none = normalizeAutomation(null);
  assert.deepEqual(effectiveAdvance({ id: 'architecture', type: 'author', locked: true }, none), { advance: 'human', set: 'human', why: 'project' });
  assert.equal(effectiveAdvance({ id: 'architecture', type: 'author', locked: true }, normalizeAutomation({ steps: { architecture: 'auto' } })).advance, 'auto');
  assert.equal(effectiveAdvance({ id: 'architecture-review', type: 'review+approve' }, normalizeAutomation({ steps: { 'architecture-review': 'auto' } })).why, 'gate',
    'a gate is human whatever the file says');
  assert.deepEqual(effectiveAdvance({ id: 'checks', automation: 'machine_advance', advance: 'auto' }, none), { advance: 'auto', set: 'auto', why: 'lane' });
  assert.equal(effectiveAdvance({ id: 'checks', automation: 'machine_advance' }, normalizeAutomation({ steps: { checks: 'human' } })).advance, 'auto',
    'a Build step reads its lane, not the project file');
  const killed = normalizeAutomation({ kill: { on: true, reason: 'incident' }, steps: { architecture: 'auto' } });
  assert.deepEqual(effectiveAdvance({ id: 'checks', automation: 'machine_advance' }, killed), { advance: 'human', set: 'auto', why: 'kill' });
  assert.equal(effectiveAdvance({ id: 'architecture' }, killed).advance, 'human');
  // A file that is there and wrong holds everything at human — the safe way to be wrong about a switch.
  for (const raw of [[1], 'x', { kill: { on: 'yes' } }, { kill: 3 }, { steps: [] }]) {
    const a = normalizeAutomation(raw);
    assert.ok(a.error, JSON.stringify(raw));
  ...
  assert.match(planShapeDial(none, { step: 'nope', to: 'auto' }).message, /unknown step: nope/);
  assert.match(planShapeDial(none, { step: 'epic', to: 'maybe' }).message, /--to must be human or auto/);
  const set = planShapeDial(none, { step: 'epic', to: 'auto' });
  assert.equal(set.changed, true);
  assert.deepEqual(serializeAutomation(set.automation), { steps: { epic: 'auto' } }, 'no kill key until somebody flips it');
  assert.equal(planShapeDial(set.automation, { step: 'epic', to: 'auto' }).changed, false);
  const back = planShapeDial(set.automation, { step: 'epic', to: 'human' });
  assert.equal(back.changed, true);
  assert.deepEqual(back.automation.steps, {}, 'human is written as the key\'s absence');
  assert.equal(planShapeDial(none, { step: 'epic', to: 'human' }).changed, false);

  const bs = { story: 'EP-x-S01', repos: {
    web: { steps: [{ id: 'checks', automation: 'human_approve', advance: 'human' }, { id: 'engineer-review', locked: true, automation: 'human_approve' }] },
    api: { status: 'skipped', record: { reason: 'no change' } },
  } };
  const opts = { story: 'EP-x-S01', declared: ['web', 'api'] };
  const lane = planLaneDial(bs, { ...opts, repo: 'web', step: 'checks', to: 'auto' });
  assert.equal(lane.changed, true);
  assert.deepEqual(lane.buildState.repos.web.steps[0], { id: 'checks', automation: 'machine_advance', advance: 'auto' }, 'both names, in place');
  assert.equal(bs.repos.web.steps[0].advance, 'human', 'the input is not mutated');
  assert.equal(planLaneDial(lane.buildState, { ...opts, repo: 'web', step: 'checks', to: 'auto' }).changed, false);
  for (const [o, message] of [
    [{ repo: 'web', step: 'engineer-review', to: 'auto' }, /merge gate, and a gate is never auto/],
    [{ repo: 'web', step: 'architecture', to: 'auto' }, /Shape step — its dial is set for the whole project/],
    [{ repo: 'web', step: 'deploy', to: 'auto' }, /unknown Build step: deploy/],
    [{ repo: 'web', step: 'checks', to: 'x' }, /--to must be human or auto/],
    [{ repo: 'mobile', step: 'checks', to: 'auto' }, /does not declare the repo mobile/],
    [{ repo: 'api', step: 'checks', to: 'auto' }, /is skipped — a skipped lane runs nothing/],
    [{ repo: 'web', step: 'spec', to: 'auto' }, /has no spec step yet/],
  ]) {
    const r = planLaneDial(bs, { ...opts, ...o });
    assert.equal(r.ok, false, JSON.stringify(o));
    assert.match(r.message, message, JSON.stringify(o));
  }

  assert.match(planKill(none, { on: true, reason: '  ' }).message, /needs a reason/);
  const k = planKill(none, { on: true, reason: ' incident ', by: 'al', date: '2026-09-15' });
  assert.deepEqual(k.automation.kill, { on: true, reason: 'incident', by: 'al', date: '2026-09-15' });
...
test('yad dial / kill / unkill write their files, show the run record, and refuse what they must (E34)', async () => {
  const { runDial, runKill } = await import('./dial.mjs');
  const { seedState: seed } = await import('./epic-state.mjs');
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e34-'));
  const E = path.join(T, 'epics/EP-x');
  fs.mkdirSync(path.join(E, '.sdlc/build-state'), { recursive: true });
  fs.mkdirSync(path.join(E, 'stories'), { recursive: true });
  fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
  fs.writeFileSync(path.join(E, '.sdlc/state.json'), JSON.stringify(seed({ epic: 'EP-x', profile: 'classic', type: 'feature', today: '2026-01-01' })));
  fs.writeFileSync(path.join(E, 'stories/EP-x-S01.md'), '---\nid: EP-x-S01\nepic: EP-x\nrepos: [web]\n---\n\n## Story\nx\n');
  const laneFile = path.join(E, '.sdlc/build-state/EP-x-S01.json');
  fs.writeFileSync(laneFile, JSON.stringify({ story: 'EP-x-S01', repos: { web: { currentStep: 'checks', steps: [
    { id: 'checks', automation: 'human_approve', advance: 'human', locked: false, status: 'todo' },
  ] } } }));
  fs.writeFileSync(path.join(E, '.sdlc/trust-log.json'), JSON.stringify({ runs: [
    { story: 'EP-x-S01', repo: 'web', step: 'checks', verdict: 'approved-unchanged' },
    { story: 'EP-x-S02', repo: 'web', step: 'checks', verdict: 'rejected' },
    { story: 'EP-x-S01', repo: 'api', step: 'checks', verdict: 'approved-unchanged' },
  ] }));
  const autoFile = path.join(T, '.sdlc/automation.json');
  const readAuto = () => JSON.parse(fs.readFileSync(autoFile, 'utf8'));
  const run = async (fn) => {
    const code = process.exitCode;
    process.exitCode = undefined;
    const out = await grab(fn);
  ...
  const lane = { epic: 'EP-x', story: 'EP-x-S01', repo: 'web', step: 'checks' };
  try {
    // A Shape author step, project-wide — and honest that nothing acts on it yet.
    let r = await run(() => runDial(T, { step: 'architecture', to: 'auto' }));
    assert.equal(r.failed, false, r.out);
    assert.deepEqual(readAuto().steps, { architecture: 'auto' });
    ...
    // A Build lane step, on the lane, with its run record as advice.
    r = await run(() => runDial(T, { ...lane, to: 'auto' }));
    assert.equal(r.failed, false, r.out);
    const row = JSON.parse(fs.readFileSync(laneFile, 'utf8')).repos.web.steps[0];
    assert.equal(row.automation, 'machine_advance');
    assert.equal(row.advance, 'auto');
    assert.match(r.out, /run record for checks in web: 2 run\(s\), 50% approved unchanged/);
    ...
    assert.equal(r.failed, true);
    assert.match(r.out, /needs a reason/);
    r = await run(() => runKill(T, { on: true, reason: 'bad deploy', today: '2026-09-15' }));
    assert.equal(r.failed, false, r.out);
    assert.equal(readAuto().kill.on, true);
    assert.equal(readAuto().kill.reason, 'bad deploy');
    assert.equal(readAuto().kill.date, '2026-09-15');
    assert.deepEqual(readAuto().steps, { architecture: 'auto' });
    r = await run(() => runDial(T, { ...lane, json: true }));
    ...
    assert.match(r.out, /already off — nothing changed/);

    // Lane refusals the CLI owns.
    r = await run(() => runDial(T, { ...lane, story: 'EP-x-S09', to: 'auto' }));
    assert.match(r.out, /no story EP-x-S09/);
    r = await run(() => runDial(T, { ...lane, repo: null, to: 'auto' }));
    assert.match(r.out, /usage: yad dial EP-x <story> --repo <name> checks/);

    // A file that cannot be read is never written over, by either command.
    fs.writeFileSync(autoFile, '{ nope');
    r = await run(() => runDial(T, { step: 'epic', to: 'auto' }));
    assert.equal(r.failed, true);
    assert.match(r.out, /nothing is written over it/);
...
test('doctor automation: silent by default; a broken file, a gate on auto and a kill_switch left in config.yaml fail (E34)', async () => {
  const { automationChecks } = await import('./doctor.mjs');
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e34d-'));
  fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
  fs.mkdirSync(path.join(T, '_bmad/sdlc'), { recursive: true });
  const found = () => { const checks = []; automationChecks(checks, T); return checks; };
  const byId = (checks, id) => checks.find((x) => x.id === id);
  try {
    fs.writeFileSync(path.join(T, '_bmad/sdlc/config.yaml'), 'automation:\n  kill_switch: false\n');
    assert.deepEqual(found(), [], 'a project that never touched automation reads exactly as before');

    fs.writeFileSync(path.join(T, '.sdlc/automation.json'), '{ nope');
    assert.equal(byId(found(), 'automation')?.status, 'fail');

    fs.writeFileSync(path.join(T, '.sdlc/automation.json'), JSON.stringify({
      kill: { on: true, reason: 'incident', by: 'al', date: '2026-09-15' },
      steps: { architecture: 'auto', 'architecture-review': 'auto', checks: 'auto', epic: 'maybe', 'from-the-future': 'auto' },
    }));
    let checks = found();
    assert.match(byId(checks, 'automation:kill').message, /ON \(by al on 2026-09-15\): incident/);
    assert.equal(byId(checks, 'automation:gate').status, 'fail');
    assert.match(byId(checks, 'automation:gate').message, /architecture-review/);
    assert.match(byId(checks, 'automation:build-step').message, /checks/);
    assert.match(byId(checks, 'automation:unknown').message, /epic: "maybe"[\s\S]*from-the-future/);
    assert.ok(!checks.some((x) => x.message.includes('architecture,') || /: architecture$/.test(x.message)), 'a Shape author step on auto is not a finding');

    // The switch's old home: set by hand there, it is OFF now, and that must not be quiet.
    fs.writeFileSync(path.join(T, '_bmad/sdlc/config.yaml'), 'automation:\n  kill_switch: true\n');
    assert.equal(byId(found(), 'automation:legacy-kill'), undefined, 'not while the real switch is on');
    fs.writeFileSync(path.join(T, '.sdlc/automation.json'), JSON.stringify({ kill: { on: false } }));
    checks = found();
...
test('yad next says when a dial is held by the kill switch, and that a Shape auto is only recorded (E34)', async () => {
  const { runNext } = await import('./next.mjs');
  const { seedState: seed } = await import('./epic-state.mjs');
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e34n-'));
  fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
  const epicWith = (id, edit) => {
    const dir = path.join(T, 'epics', id);
    fs.mkdirSync(path.join(dir, '.sdlc/build-state'), { recursive: true });
    const state = seed({ epic: id, profile: 'classic', type: 'feature', today: '2026-01-01' });
    edit(state, dir);
    fs.writeFileSync(path.join(dir, '.sdlc/state.json'), JSON.stringify(state));
  };
  // A Shape author step open now: epic approved, architecture being written.
  epicWith('EP-shape', (state) => {
    for (const st of state.steps) if (st.id === 'epic' || st.id === 'epic-review') st.status = 'done';
    state.steps.find((st) => st.id === 'architecture').status = 'in_progress';
    state.currentStep = 'architecture';
  });
  // A Build lane whose `checks` step is on auto.
  epicWith('EP-build', (state, dir) => {
    for (const st of state.steps) st.status = 'done';
    state.currentStep = 'ready-for-build';
    fs.writeFileSync(path.join(dir, '.sdlc/build-state/EP-build-S01.json'), JSON.stringify({ story: 'EP-build-S01', repos: { web: { currentStep: 'checks', steps: [
      { id: 'spec', automation: 'human_approve', advance: 'human', status: 'done' },
      { id: 'checks', automation: 'machine_advance', advance: 'auto', status: 'todo' },
      { id: 'engineer-review', automation: 'human_approve', advance: 'human', locked: true, status: 'todo' },
    ] } } }));
  });
  const auto = (body) => fs.writeFileSync(path.join(T, '.sdlc/automation.json'), JSON.stringify(body));
  const next = (epic) => grab(() => runNext(T, { epic }));
  try {
    assert.doesNotMatch(await next('EP-shape'), /advance: auto/, 'no line at all while nobody set one');
    assert.match(await next('EP-build'), /advance: auto — yad-run auto-drives/);

    auto({ steps: { architecture: 'auto' } });
    assert.match(await next('EP-shape'), /advance: auto — recorded; nothing drives a Shape step on its own yet/);

    auto({ kill: { on: true, reason: 'incident' }, steps: { architecture: 'auto' } });
    const shape = await next('EP-shape');
    assert.match(shape, /held at human by the kill switch \(yad unkill\)/);
    assert.doesNotMatch(shape, /recorded; nothing drives/);
...
test('yad epic new --stub: an anchor for a feature that shipped before the Product existed', async () => {
  ...
  try {
    ...
    assert.deepEqual([...new Set(st.steps.map((s) => s.status))], ['todo']);
...
// ---- skill binding — which skill runs a step is the project's choice (E6) -------------------------
//
// The engine still ships a default for every step. What is new is that the default is no longer the
// only possible answer: `.sdlc/skills.json` overrides it, and several skills on one step run as a
// chain (closed decision 7). Every test here feeds the resolver a binding that DISAGREES with the
...
test('a binding is read as a list whatever it was written as, and junk is dropped', () => {
  // One string and a list of them are the two spellings a person will actually write, and both come
  // back as a list so nothing downstream has to handle two shapes.
  assert.deepEqual(normalizeBindings({ steps: { epic: 'mine' } }).steps, { epic: ['mine'] });
  assert.deepEqual(normalizeBindings({ steps: { epic: ['a', 'b'] } }).steps, { epic: ['a', 'b'] });
  assert.deepEqual(normalizeBindings({ steps: { epic: ['  a  '] } }).steps, { epic: ['a'] }, 'trimmed');

  // Junk is DROPPED, never thrown on: this runs inside `yad next`, which must still say what to do
  // next on a project whose config file someone mistyped. Each of these leaves the step on its
  // default, and `yad doctor` is what reports that the line did nothing.
  for (const junk of [null, undefined, 42, 'a string', [], { steps: null }, { steps: [] }, { steps: 'x' }]) {
    assert.deepEqual(normalizeBindings(junk).steps, {}, `${JSON.stringify(junk)} binds nothing`);
  }
  for (const value of ['', '   ', [], [''], ['  '], 7, null, {}, [7, null]]) {
    assert.deepEqual(normalizeBindings({ steps: { epic: value } }).steps, {},
      `${JSON.stringify(value)} is not a skill name`);
  }
...
test('stepSkills asks the project first and falls back to the catalogue', () => {
  const bound = { steps: { epic: ['mine'], implement: ['first', 'second'] } };
  // The project's answer, which is NOT the catalogue's — a resolver ignoring the file would say
  // yad-epic here and still pass a test that bound `epic` to `yad-epic`.
  assert.deepEqual(stepSkills('epic', bound), ['mine']);
  assert.deepEqual(stepSkills('implement', bound), ['first', 'second']);
  // Unbound steps, Shape and Build alike, keep the shipped default.
  assert.deepEqual(stepSkills('architecture', bound), ['yad-architecture']);
  assert.deepEqual(stepSkills('checks', bound), ['yad-checks']);
  assert.deepEqual(stepSkills('architecture', null), ['yad-architecture'], 'no file at all is the normal case');
  // A Shape review gate honestly has no skill — it is driven by `yad gate`, not by one.
  assert.deepEqual(stepSkills('architecture-review', bound), []);
  assert.deepEqual(stepSkills('not-a-step', bound), []);
  ...
  list.push('oops');
  assert.deepEqual(stepSkills('epic', bound), ['mine'], 'the binding was mutated through its own return value');
});

test('`skills` appears only for a chain, so an unbound project emits the same JSON it always did', () => {
  // `yad next --json` is deep-equalled by the golden test against a frozen v3 project, and rule 6 says
  // a frozen project's answers never change — a key that appeared on EVERY action would break it. So
  // this is not a `parallel`-style always-present field, and that departure is the point.
  const one = skillFields(['only']);
  assert.deepEqual(one, { skill: 'only' });
  assert.equal('skills' in one, false, 'a single skill must not add a second key');
  assert.deepEqual(skillFields(['a', 'b']), { skill: 'a', skills: ['a', 'b'] });
  const none = skillFields([]);
  assert.deepEqual(none, { skill: null });
  assert.equal('skills' in none, false);
});

test('loadSkillBindings reads the project file, and an absent one binds nothing', () => {
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e6-load-'));
  try {
    assert.deepEqual(loadSkillBindings(T).steps, {}, 'no file is the normal case, not an error');
    fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
    fs.writeFileSync(path.join(T, '.sdlc/skills.json'), JSON.stringify({ steps: { stories: 'mine' } }));
    assert.deepEqual(loadSkillBindings(T).steps, { stories: ['mine'] });
    // Unreadable is the same as absent HERE on purpose: `yad next` still answers, and `yad doctor` is
    // the surface that reports the broken file.
    fs.writeFileSync(path.join(T, '.sdlc/skills.json'), '{ not json');
    assert.deepEqual(loadSkillBindings(T).steps, {});
  } finally { fs.rmSync(T, { recursive: true, force: true }); }
});

test('nextAction names the bound skill for every kind of action it resolves', () => {
  const bindings = { steps: { epic: ['mine'], architecture: ['a', 'b'], 'test-cases': ['tc1', 'tc2'], discovery: ['disc'] } };
  const chain = (ids, currentStep) => ({
    state: {
      epicId: 'EP-x', currentStep,
  ...
  assert.equal(nextActionE6(null, { epic: 'EP-x', bindings }).skill, 'mine');
  assert.equal(nextActionE6(null, { epic: 'EP-x' }).skill, 'yad-epic', 'and the default without a binding');

  // A plain author step, bound to a chain.
  const arch = nextActionE6(chain([
    ['epic', 'author', 'epic.md', 'done'],
    ['architecture', 'author', 'architecture.md', 'in_progress'],
  ], 'architecture'), { epic: 'EP-x', bindings });
  assert.equal(arch.skill, 'a');
  assert.deepEqual(arch.skills, ['a', 'b']);

  // The parallel test-cases track resolves through the same binding, not through the catalogue.
  const par = nextActionE6(chain([
    ['architecture', 'author', 'architecture.md', 'in_progress'],
    ['test-cases', 'author', 'test-cases.md', 'in_progress'],
  ], 'architecture'), { epic: 'EP-x', bindings });
  assert.equal(par.parallel.skill, 'tc1');
  assert.deepEqual(par.parallel.skills, ['tc1', 'tc2']);

  // The product front-zero has its own branch in the resolver and must not miss the binding.
  ...
  // And with no bindings at all nothing gains a key — the unbound project's JSON is untouched.
  const plain = nextActionE6(chain([['architecture', 'author', 'architecture.md', 'in_progress']], 'architecture'), { epic: 'EP-x' });
  assert.equal(plain.skill, 'yad-architecture');
  assert.equal('skills' in plain, false);
});

test('a Build lane names the bound skill, and its remaining chain does not repeat it', () => {
  const repo = { currentStep: 'implement', steps: [
    { id: 'spec', status: 'done' }, { id: 'tasks', status: 'done' },
    { id: 'implement', status: 'in_progress' }, { id: 'checks', status: 'todo' },
    { id: 'engineer-review', status: 'todo' },
  ] };
  const bindings = { steps: { implement: ['build-a', 'build-b'] } };
  const lane = buildNextE6(repo, { bindings });
  assert.equal(lane.skill, 'build-a');
  assert.deepEqual(lane.skills, ['build-a', 'build-b']);
  // The chain is every skill still to run, this step's own included — so both bound skills lead it,
  // then the rest of the lane on its defaults.
  assert.deepEqual(lane.chain, ['build-a', 'build-b', 'yad-checks', 'yad-engineer-review']);

  // Unbound, the output is byte-for-byte what it was before E6 — including no `skills` key, which the
  // golden test would otherwise reject.
  const plain = buildNextE6(repo);
  assert.equal(plain.skill, 'yad-implement');
  assert.equal('skills' in plain, false);
  assert.deepEqual(plain.chain, ['yad-implement', 'yad-checks', 'yad-engineer-review']);
  // spec and tasks are two legs of one ceremony and still collapse to a single entry.
...
function skillProject(files = {}) {
  const T = fs.mkdtempSync(path.join(os.tmpdir(), 'sdlc-e6-cmd-'));
  fs.mkdirSync(path.join(T, '.sdlc'), { recursive: true });
  for (const [rel, body] of Object.entries(files)) {
    fs.mkdirSync(path.dirname(path.join(T, rel)), { recursive: true });
...
test('yad next prints a bound chain, and says what the extra skills cost', async () => {
  const T = skillProject({ '.sdlc/skills.json': { steps: { architecture: ['ours', 'yad-architecture'], implement: ['build-a', 'build-b'] } } });
  try {
    seedEpic(T, 'EP-x', { epicId: 'EP-x', currentStep: 'architecture', steps: [
      { id: 'epic', type: 'author', artifact: 'epic.md', status: 'done' },
      { id: 'architecture', type: 'author', artifact: 'architecture.md', status: 'in_progress' },
    ] });
    const s = await grab(() => runNext(T, { epic: 'EP-x' }));
    assert.match(s, /invoke the ours skill, then yad-architecture/);
    // Closed decision 7's cost warning, printed where the chain is about to be invoked.
    assert.match(s, /2 skills run for this step/);

    // …and an UNBOUND step reads exactly as it always did, with no cost line. A warning printed on
    // every step would be noise on the case that is not a chain at all.
    seedEpic(T, 'EP-u', { epicId: 'EP-u', currentStep: 'stories', steps: [
      { id: 'stories', type: 'author', artifact: 'stories/', status: 'in_progress' },
    ] });
    const u = await grab(() => runNext(T, { epic: 'EP-u' }));
    assert.match(u, /invoke the yad-stories skill/);
    assert.doesNotMatch(u, /skills run for this step/);

    // A Build lane: the bound chain leads the line, and the "then →" tail must NOT repeat its own
    // second skill as if it came later in the lane.
    seedEpic(T, 'EP-b', { epicId: 'EP-b', currentStep: 'ready-for-build', steps: [
      { id: 'epic', type: 'author', artifact: 'epic.md', status: 'done' },
    ] });
    fs.mkdirSync(path.join(T, 'epics/EP-b/.sdlc/build-state'), { recursive: true });
    fs.writeFileSync(path.join(T, 'epics/EP-b/.sdlc/build-state/EP-b-S01.json'), JSON.stringify({
      story: 'EP-b-S01',
      repos: { api: { currentStep: 'implement', steps: [
        { id: 'spec', status: 'done' }, { id: 'implement', status: 'in_progress' },
        { id: 'checks', status: 'todo' }, { id: 'engineer-review', status: 'todo' },
      ] } },
    }));
    const b = await grab(() => runNext(T, { epic: 'EP-b' }));
    assert.match(b, /invoke the build-a skill, then build-b/);
...
test('`yad epic new` hands the first step to the skill the project bound, not the catalogue default', async () => {
  const { runEpicNew } = await import('./epic.mjs');
  const T = skillProject({ '.sdlc/skills.json': { steps: { epic: ['shape-it', 'yad-epic'] } } });
  try {
    const out = await grab(() => runEpicNew(T, { slug: 'demo', today: '2026-01-02', json: true }));
    const j = JSON.parse(out);
  ...
  try {
    const j = JSON.parse(await grab(() => runEpicNew(U, { slug: 'demo', today: '2026-01-02', json: true })));
    assert.equal(j.next, 'yad-epic');
    assert.equal('nextSkills' in j, false);
  } finally { fs.rmSync(U, { recursive: true, force: true }); }

  // A stub is woken by `yad-backfill promote`, which is an engine verb and not a step on any chain —
  // so it is named even when the project has bound `epic` to something else.
  const S = skillProject({ '.sdlc/skills.json': { steps: { epic: ['shape-it'] } } });
  try {
    const j = JSON.parse(await grab(() => runEpicNew(S, { slug: 'legacy', stub: true, today: '2026-01-02', json: true })));
    assert.equal(j.next, 'yad-backfill');
  } finally { fs.rmSync(S, { recursive: true, force: true }); }
});

test('doctor reports a skill binding that does nothing, and corrects none of them', async () => {
  const { skillBindingChecks } = await import('./doctor.mjs');
  const run = (body) => {
    const T = skillProject(body === null ? {} : { '.sdlc/skills.json': body });
    const checks = [];
    const before = body === null ? null : fs.readFileSync(path.join(T, '.sdlc/skills.json'), 'utf8');
    try {
      skillBindingChecks(checks, T);
      if (before !== null) {
        assert.equal(fs.readFileSync(path.join(T, '.sdlc/skills.json'), 'utf8'), before, 'doctor rewrote the file');
      }
      return checks;
    } finally { fs.rmSync(T, { recursive: true, force: true }); }
  };
  const ids = (checks) => checks.map((c) => `${c.status}:${c.id}`);

  // Absent is the normal case — most projects run the shipped skills, and a permanent nag about that
  // would be a warning people learn to skip past.
  assert.deepEqual(run(null), []);

  assert.deepEqual(ids(run('{ not json')), ['fail:skills']);
  assert.deepEqual(ids(run([1, 2])), ['fail:skills']);
  assert.deepEqual(ids(run({ steps: [] })), ['fail:skills']);
  // A file holding only its shape binds nothing, correctly — that is what unbinding the last line
  // leaves behind.
  assert.deepEqual(run({ schemaVersion: ENGINE_SHAPE }), []);

  // A value that names no skill: the line sits in the file and does nothing, which is the one failure
  // a person editing JSON by hand would never guess.
  const unusable = run({ steps: { epic: '', stories: 7 } });
  assert.deepEqual(ids(unusable), ['warn:skills']);
  ...
  assert.match(unknown.find((c) => c.id === 'skills:unknown-step').message, /relase/);

  // A review gate: known to the engine, but no skill ever runs it.
  const gate = run({ steps: { 'architecture-review': 'x' } });
  assert.ok(gate.some((c) => c.id === 'skills:review-step' && c.status === 'warn'));
