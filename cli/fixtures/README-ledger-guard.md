...
such a Product as verified, the audit trail silently stops being enforced — the one guarantee verified
mode exists to provide.

`cli/test-checks.mjs` runs it against a migrated Product and asserts it still arms. That is what makes
